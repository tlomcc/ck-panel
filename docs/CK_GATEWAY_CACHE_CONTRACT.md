...
```json
{
  ...
  "upstream_format": "anthropic for a provider with api_type=claude, openai for api_type=openai",
...
- `native_stable`: use the native stable system + latest completed assistant two-anchor shape. When the selected provider has `api_type: "claude"`, the gateway sends Anthropic `/messages`; an OpenAI provider keeps its OpenAI wire format. The strategy sends `prompt_cache_ttl: "1h"` and leaves the current user, time, recall, and reply-target blocks after the assistant anchor.
- `native_tiered`: use the three `1h` anchors (stable system plus latest two completed assistants) and a final `5m` anchor on the latest real user text. The provider's `api_type` still decides whether those anchors are serialized as Claude Messages or OpenAI Chat Completions.
- `native_5m`: use the Claude Code breakpoint shape — stable system, last tool, previous real user text (read anchor) and current real user text (creation anchor), all `5m`. Breakpoints follow the conversation tail, so the current turn's own question also enters the cache and the next turn can read it; a pause longer than 5 minutes rebuilds everything. The provider's `api_type` decides the wire format, and the gateway pins the TTL at `5m` for this strategy.
