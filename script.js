...
function ckCloseDialog(value){
  ...
  var returnFocus=ckDialogState.returnFocus;
...
function ckDialogSubmit(){
  ...
  if(ckDialogState.mode==='prompt'){
    ...
    if(ckDialogState.required&&!value){if(input)input.focus();return}
    ckCloseDialog(value);
    return;
  }
  ckCloseDialog(true);
...
document.addEventListener('keydown',function(e){
  ...
  if(e.key==='Tab'){
    var focusable=Array.from(modal.querySelectorAll('button:not([disabled]),input:not([disabled]):not([hidden])')).filter(function(el){return el.offsetParent!==null});
...
function syncPanelVersionBadge(){
  ...
  var debugEl=document.getElementById('chat-debug-version');
  if(debugEl){
    debugEl.textContent=shortVersion;
    debugEl.title='当前版本 '+CK_PANEL_VERSION;
  }
}
...
function decodeJsonStringPrefix(text,quoteAt){
  text=String(text||'');
  var out='',esc=false;
  for(var i=quoteAt+1;i<text.length;i++){
...
function extractJsonContentString(text){
  text=String(text||'');
  var re=/"content"\s*:\s*"/g,match;
  while((match=re.exec(text))){
    var quoteAt=re.lastIndex-1;
    var content=decodeJsonStringPrefix(text,quoteAt);
    if(content)return content;
  }
  return '';
}
function normalizeMemoryMarkdownText(text){
  return String(text||'').replace(/\r\n/g,'\n').replace(/\r/g,'\n').replace(/\n{0,2}\[truncated: \d+ chars omitted\]\s*$/,'');
}
function mcpUnwrapStructuredText(text){
  ...
  var content=extractJsonContentString(trimmed);
  if(content)return content;
  return text;
}
...
var currentPanelTab='chat';
var factLibraryData=null,factLibraryItems=[],factLibraryLoading=false,factLibraryRequestId=0,factLibraryOffset=0,factLibraryHasMore=false,factLibrarySearchTimer=null,factLibrarySort='recent',factDetailItem=null,factDetailSaving=false;
var archiveOverviewData={facts:null},archiveOverviewLoading=false,archiveOverviewRequestId=0;
var EMPTY_CATEGORY_MARKER='[ck-panel-empty]';
function memoryRawContent(raw){
  if(raw==null)return '';
  if(typeof raw==='object'){
    if(typeof raw.content==='string')return normalizeMemoryMarkdownText(raw.content);
    if(raw.result&&typeof raw.result.content==='string')return normalizeMemoryMarkdownText(raw.result.content);
  ...
  if(/^[\[{]/.test(trimmed)){
    try{
      var data=JSON.parse(trimmed);
      if(data&&typeof data==='object'){
        if(typeof data.content==='string')return normalizeMemoryMarkdownText(data.content);
        if(data.result&&typeof data.result.content==='string')return normalizeMemoryMarkdownText(data.result.content);
      }
    }catch(e){}
    var content=extractJsonContentString(trimmed);
    if(content)return normalizeMemoryMarkdownText(content);
  }
  return normalizeMemoryMarkdownText(text);
}
function timeAgo(dateStr){if(!dateStr)return'';var d=new Date(dateStr);if(isNaN(d.getTime())){var norm=String(dateStr).replace(/[.\/]/g,'-').replace(/-+/g,'-').replace(/-$/,'');d=new Date(norm)}if(isNaN(d.getTime()))return'';var now=new Date(),diff=Math.floor((now-d)/864e5);if(diff<=0)return'今天';if(diff===1)return'昨天';if(diff<30)return diff+'天前';if(diff<365)return Math.floor(diff/30)+'个月前';return Math.floor(diff/365)+'年前'}
function daysSince(){return Math.floor((new Date()-new Date(2026,2,26))/864e5)}
function dateKey(d){return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0')}
function normalizeDateKey(s){return String(s||'').slice(0,10).replace(/[./]/g,'-')}
function init(){
...
function startPanelDataApp(){
  ...
  // 启动遮罩以前要等 MCP 记忆分类全部读完才收；那套隐藏页已经删掉了，
  // 现在首屏 tab 切好就可以收。别删这两行，否则面板会永远停在启动画面。
  setLoading(100,'已就绪');
  hideLoadingSoon(120);
}
...
function renderArchiveFactOverview(facts){
  facts=facts||{};
  ...
  var generation=String(facts.generation||'');
  archiveOverviewSetText('archive-overview-indexed','向量 generation：'+(generation?generation.slice(0,12):'尚无记录'));
...
function archiveOverviewFetch(force){
  ...
  return panelDataFetch(factUrl,{cache:'no-store'},{label:'CK 事实与向量总览'}).then(function(response){
    if(!response.ok)throw new Error('事实接口 HTTP '+response.status);
    return response.json();
  }).then(function(data){
    if(!data||!Array.isArray(data.items)||!data.counts)throw new Error('事实数据格式无效');
    return {facts:data};
  });
}
function loadArchiveFactOverview(force){
  ...
  if(!force&&archiveOverviewData.facts){renderArchiveFactOverview(archiveOverviewData.facts);return Promise.resolve(archiveOverviewData)}
  var requestId=++archiveOverviewRequestId;
  archiveOverviewLoading=true;
  archiveOverviewLoadingState(force?'正在刷新事实与向量总览…':'正在读取事实与向量总览…');
  return archiveOverviewFetch(force).then(function(data){
    ...
    renderArchiveFactOverview(data.facts);
    return data;
  }).catch(function(err){
    if(requestId!==archiveOverviewRequestId)return archiveOverviewData;
    if(archiveOverviewData.facts){
      renderArchiveFactOverview(archiveOverviewData.facts);
      ...
      ['archive-field-list','archive-recent-facts','archive-health-grid'].forEach(function(id){var el=document.getElementById(id);if(el)el.innerHTML='<div class="facts-empty facts-empty-error"><b>暂时读不到</b><span>不会修改任何事实或向量数据。</span></div>'});
    }
...
function factFacetOptions(id,facets,allLabel){
  var el=document.getElementById(id);if(!el)return;
  var selected=String(el.value||'');
  var html='<option value="">'+esc(allLabel)+'</option>';
  (Array.isArray(facets)?facets:[]).forEach(function(item){
    var value=String(item.value||''),label=String(item.label||value),count=numOr(item.count,0);
    html+='<option value="'+escAttr(value)+'">'+esc(label)+' · '+count+'</option>';
  });
  el.innerHTML=html;
...
function renderFactLibrary(data){
  ...
  renderFactStats(data||{});
  renderFactCards();
  var more=document.getElementById('facts-more'),hint=document.getElementById('facts-more-hint');
...
function onFactSearch(value){
  var clear=document.getElementById('facts-query-clear');
  if(clear)clear.classList.toggle('show',!!String(value||'').trim());
  if(factLibrarySearchTimer)clearTimeout(factLibrarySearchTimer);
  factLibrarySearchTimer=setTimeout(function(){loadFactLibrary(true,false)},260);
}
function clearFactSearch(){
  var input=document.getElementById('facts-query');if(input)input.value='';
...
var CHAT_FOLDERS_KEY='ckChatFoldersV1';
...
var chatStaggeredMessageKeys=new Set();
var chatMessageRenderMemo=new WeakMap();
var chatAssistantRevealQueue=null;
var chatPendingMessageSeq=0;
var chatPendingGesture={row:null,message:null,startX:0,startY:0,dx:0,pointerId:null,pressTimer:0,moved:false,longPressed:false};
var chatPendingSuppressClickUntil=0;
...
function chatShowCleanHistoryConfirm(){
  // 原来这里挂着一个自己的 .confirm-bg 弹窗（和已删除的记忆分类页共用那套样式）。
  // 现在统一走面板共用的 ckConfirmDialog，样式和其他确认框一致。
  return ckConfirmDialog('将清理历史消息中的图片和召回内容。',{title:'清理历史',confirmText:'确认',danger:true});
}
...
function chatStripLocalHistoryMediaAndRecall(targetMessages){
  var stats={images:0,recalls:0,changed:false};
  (targetMessages||chatMessages||[]).forEach(function(m){
    if(!m||typeof m!=='object')return;
...
async function chatCleanHistoryCore(cfg){
  ...
  try{
    ...
    if(Array.isArray(data.transport_messages)){
      targetSession.transportMessages=chatLimitArray(data.transport_messages,CHAT_MAX_TRANSPORT_MESSAGES);
      targetSession.transportUpdated=Date.now();
    }else{
      targetSession.transportMessages=[];
      targetSession.transportUpdated=0;
    }
...
function chatNormalizeAutoTrimConfig(raw){
  ...
  var roundLimit=chatPositiveIntOrDefault(
    raw.roundLimit!==undefined?raw.roundLimit:(raw.round_limit!==undefined?raw.round_limit:raw.maxRounds),
    CHAT_AUTO_TRIM_DEFAULT_ROUND_LIMIT
  );
  return {
...
/* ---- 计费总闸 + 每条 API 自己的价格 ---- */
// 分工（2026-08-23 用户定的）：
//   设置页那个开关＝总闸。关掉聊天面板一律不显示价格，轮询页的"显示价格"也压不过它。
//   单价不再在这里填，改成在「聊天轮询」页给每条 API 各维护一份，谁答的就按谁的价算。
// 兜底很重要：没维护过的 API 用面板默认单价，否则改造完用户会以为价格功能坏了。
//
...
function chatApiPricingToCostPricing(raw,base){
  base=base||chatDefaultCostPricing();
  raw=(raw&&typeof raw==='object')?raw:{};
  var num=function(value,fallback){
    var n=Number(value);
    return (isFinite(n)&&n>=0)?n:fallback;
  ...
  var pick=function(a,b){return a!==undefined&&a!==null&&a!==''?a:b};
  var create=num(pick(raw.cache_create,raw.cacheCreate),base.cacheCreate5mPerMTokens);
  return {
    mode:base.mode,
    currency:String(raw.currency||base.currency||'¥').trim()||'¥',
    inputPerMTokens:num(pick(raw.input,raw.inputPerMTokens),base.inputPerMTokens),
    outputPerMTokens:num(pick(raw.output,raw.outputPerMTokens),base.outputPerMTokens),
    cacheReadPerMTokens:num(pick(raw.cache_read,raw.cacheRead),base.cacheReadPerMTokens),
    cacheCreate5mPerMTokens:create,
    cacheCreate1hPerMTokens:create,
    cacheCreateLegacyPerMTokens:create,
    ttlCreatePricingConfigured:true,
    multiplier:num(raw.multiplier,base.multiplier)
  };
}
...
function chatUsageCost(usage){
  ...
  // 单价按"这一轮是谁答的"取：轮询页给每条 API 各维护一份，没维护的落回面板默认价。
  var pricing=chatPricingForUsage(usage);
...
function chatAttachAssistantCost(msg,usage){
  ...
  // 用量统计那一行要的原始 token。以前只存了钱，token 数没留下来，
  // 刷新一次页面就再也算不出命中率了，所以这里一并落盘。
  if(cost.hasUsage){
    msg.tkIn=cost.input;
    msg.tkInTotal=cost.inputTotal;
    msg.tkOut=cost.output;
    msg.tkRead=cost.read;
    msg.tkCreate=cost.create;
  }
  return msg;
}
function chatAssistantCostLabel(msg){
  ...
  // 用户明确要求：聊天面板直接给价格数字，前面不再加任何来源前缀。
  return chatCostAmountText(value,msg.apiCostCurrency||'');
}
...
function chatNormalizeThinkingBudget(value){
  var n=Math.round(Number(value));
  if(!isFinite(n))return 4096;
...
function chatRenderThinkingControls(cfg){
  cfg=cfg||chatLoadConfig()||{};
  var mode=chatNormalizeThinkingMode(cfg.thinkingMode,cfg.fakeThinking===true);
  var modeInput=document.getElementById('chat-thinking-mode');
...
function chatThinkingModeChanged(value){
  var mode=chatNormalizeThinkingMode(value);
  var fake=document.getElementById('chat-fake-thinking');
  if(fake)fake.checked=mode==='compat';
  var cfg=chatSaveConfig(true);
  chatRenderThinkingControls(cfg);
  return cfg;
}
function chatCompatThinkingChanged(enabled){
  var modeInput=document.getElementById('chat-thinking-mode');
...
function chatCacheNoticeStrategy(cfg){
  ...
  if(pollingOn){
    ...
    var activeId=String(live.active_provider_id||live.provider_id||'').trim();
    if(activeId){
      var active=findLibraryProvider(activeId);
      var activeStrategy=providerCacheStrategy(active);
      if(activeStrategy)return activeStrategy;
    }
    // 轮询状态还没回到聊天页时，供应商级策略未知，不能拿聊天页旧策略
    // 伪装成当前实际生效的 TTL。
    return '';
  }
  var bound=providerNormalizeCacheStrategy(cfg.mainRouteCacheStrategy);
  return bound||chatNormalizeCacheStrategy(cfg.cacheStrategy);
}
...
function chatNormalizeRecallRecentRounds(value){
  ...
  return isFinite(number)?Math.max(0,Math.min(100,number)):10;
}
function chatNormalizeTimeInjectionEveryRounds(value){
  var number=Math.round(Number(value));
  return isFinite(number)?Math.max(1,Math.min(1000,number)):1;
}
...
function chatFactRecallModeFromForm(fallback){
  ...
  return chatNormalizeFactRecallMode(selected?selected.value:fallback);
}
function chatSetFactRecallModeField(value){
  var mode=chatNormalizeFactRecallMode(value);
  document.querySelectorAll('input[name="chat-fact-recall-mode"]').forEach(function(input){
    input.checked=input.value===mode;
  });
  return mode;
}
function chatRecallModeMeta(value){
  return {value:'fact_only',label:'Fact 召回',shortLabel:'Fact',debugText:'只检索并注入有效独立 Fact'};
}
...
function chatRenderRecallState(statusText,statusKind){
  ...
  var factMode=chatFactRecallModeMeta(cfg.factRecallMode);
  ...
  chatSetFactRecallModeField(factMode.value);
  ...
  var factControl=document.getElementById('chat-fact-recall-mode-control');
  if(factControl){
    factControl.classList.toggle('is-disabled',!meta.enabled);
    factControl.setAttribute('aria-disabled',meta.enabled?'false':'true');
  }
  document.querySelectorAll('input[name="chat-fact-recall-mode"]').forEach(function(radio){radio.disabled=!meta.enabled});
  var detail=document.getElementById('chat-recall-mode-detail');
  if(detail)detail.textContent=meta.enabled
    ? '当前状态：'+factMode.label+'｜'+factMode.debugText
    : '当前状态：完全关闭，不读取或注入 Fact';
...
function chatRenderNcContextState(statusText,statusKind){
  ...
  var every=chatNormalizeTimeInjectionEveryRounds(cfg.timeInjectionEveryRounds);
  var input=document.getElementById('chat-nc-context-injection');
  if(input)input.checked=enabled;
  var rounds=document.getElementById('chat-time-injection-every-rounds');
  if(rounds)rounds.value=String(every);
  var status=document.getElementById('chat-nc-context-save-status');
  if(status){
    status.textContent=statusText||('已保存：'+(enabled?'开启｜每 '+every+' 轮注入当前时间':'关闭｜不注入当前时间'));
    status.className='chat-cache-save-status'+(statusKind?' '+statusKind:'');
  }
}
function chatRenderBackendSwitchNotificationState(statusText,statusKind){
  var cfg=chatLoadConfig()||{};
  var enabled=cfg.backendSwitchNotification!==false;
  var input=document.getElementById('chat-backend-switch-notification');
  if(input)input.checked=enabled;
  var status=document.getElementById('chat-backend-switch-save-status');
  if(status){
    status.textContent=statusText||('已保存：'+(enabled?'开启｜切换时通知':'关闭｜切换时静默'));
    status.className='chat-cache-save-status'+(statusKind?' '+statusKind:'');
  }
}
...
function chatMainRouteConfig(){
  ...
  var route={
    ...
    model:model,
    apiType:p?providerNormalizeApiType(p.api_type,p.url):'openai',
    source:'api_config_main_io',
    reason:''
  };
...
function chatApplyMainRouteToConfig(cfg,route){
  ...
  if(route.ok){
    ...
    cfg.mainRouteApiType=providerNormalizeApiType(route.apiType||((route.provider||{}).api_type),route.apiBase);
...
function chatLoadConfig(){
  ...
  cfg.timeInjectionEveryRounds=chatNormalizeTimeInjectionEveryRounds(cfg.timeInjectionEveryRounds);
  ...
  cfg.factRecallMode=chatNormalizeFactRecallMode(cfg.factRecallMode);
  cfg.recallRecentRounds=chatNormalizeRecallRecentRounds(cfg.recallRecentRounds);
  cfg.thinkingMode=chatNormalizeThinkingMode(
    Object.prototype.hasOwnProperty.call(saved||{},'thinkingMode')?cfg.thinkingMode:'',
...
function chatMergeLiveToggleState(cfg){
  ...
  cfg.factRecallMode=chatFactRecallModeFromForm(cfg.factRecallMode);
  cfg.recallRecentRounds=chatNormalizeRecallRecentRounds(chatFieldValue('chat-recall-recent-rounds',cfg.recallRecentRounds));
  return cfg;
}
...
function chatImageFingerprint(dataUrl){
  dataUrl=String(dataUrl||'');
  if(!dataUrl)return '';
  var len=dataUrl.length;
  // JPEG/PNG headers are near-identical across images; sample the tail where real pixel data differs.
  var tail=dataUrl.slice(-64);
  return 'img'+len.toString(36)+'-'+tail.replace(/[^A-Za-z0-9]/g,'').slice(-40);
}
function chatNormalizeImageAttachment(img){
  ...
  return {
    id:String(img.id||img.key||chatImageFingerprint(dataUrl)||chatImageId()).slice(0,96),
...
function chatInvalidateTransportForEdit(){
  ...
  if(!s)return;
  if((s.transportMessages&&s.transportMessages.length)||s.transportUpdated){
    s.transportMessages=[];
...
function chatReadForm(){
  ...
  var cfg={
    ...
    factRecallMode:chatFactRecallModeFromForm(saved.factRecallMode),
    recallRecentRounds:chatNormalizeRecallRecentRounds(chatFieldValue('chat-recall-recent-rounds',saved.recallRecentRounds)),
    thinkingMode:chatNormalizeThinkingMode(
      chatFieldValue('chat-thinking-mode',saved.thinkingMode),
      saved.fakeThinking===true,
    ),
...
function chatSaveRecallSetting(auto){
  ...
  if(!auto)toast('Fact 召回已保存：'+meta.label);
  return cfg;
}
function chatSaveNcContextInjection(auto){
  ...
  var enabled=cfg.ncContextInjection!==false;
  var every=chatNormalizeTimeInjectionEveryRounds(cfg.timeInjectionEveryRounds);
  chatRenderNcContextState('已保存成功：'+(enabled?'开启｜每 '+every+' 轮注入当前时间':'关闭｜不注入当前时间'),'ok');
  if(!auto)toast('当前时间上下文已'+(enabled?'开启':'关闭'));
  return cfg;
}
function chatSaveBackendSwitchNotification(auto){
  var cfg=chatSaveConfig(true);
  var enabled=cfg.backendSwitchNotification!==false;
  chatRenderBackendSwitchNotificationState('已保存成功：'+(enabled?'开启｜切换时通知':'关闭｜切换时静默'),'ok');
  if(!auto)toast('后端切换通知已'+(enabled?'开启':'关闭'));
  return cfg;
}
...
function chatRenderTickLegend(){
  ...
  if(!box)return;
  box.innerHTML=CHAT_TICK_LEGEND_ROWS.map(function(row){
    var hit=row[0]==='full'||row[0]==='partial';
    return '<div class="chat-tick-legend-row">'+
...
function chatSaveFactRecallMode(auto){
  var cfg=chatSaveConfig(true);
  var modeMeta=chatFactRecallModeMeta(cfg.factRecallMode);
  chatRenderRecallState('已保存成功：召回模式 '+modeMeta.label+'｜'+modeMeta.debugText,'ok');
  if(!auto)toast('召回模式已保存：'+modeMeta.label);
  return cfg;
}
function chatSetFactRecallMode(value,auto){
  chatSetFactRecallModeField(value);
  return chatSaveFactRecallMode(auto);
}
...
function chatRenderSpeechPreferences(data,preserveEditor){
  ...
  var stale=document.getElementById('chat-speech-stale-dot');
  if(stale)stale.hidden=data.draft_stale!==true;
}
/* ---- 规则管理页 ---- */
...
function chatSpeechPreferencesEnabledForTrim(){
  return !!(rulesPageState&&rulesPageState.data&&rulesPageState.data.enabled!==false);
}
async function chatEnsureSpeechPreferenceState(){
  if(rulesPageState&&rulesPageState.data)return chatSpeechPreferencesEnabledForTrim();
  if(rulesPageState&&rulesPageState.loading)return false;
  var loaded=await loadRulesPage(false);
  return loaded===true&&chatSpeechPreferencesEnabledForTrim();
}
...
function rulesMoveRow(button,delta){
  var row=button&&button.closest?button.closest('.rules-row'):null;
  if(!row)return;
  var sibling=delta<0?row.previousElementSibling:row.nextElementSibling;
  if(!sibling||!sibling.classList||!sibling.classList.contains('rules-row'))return;
...
function rulesUsePublishedDraft(){
  var data=rulesPageState.data||{};
  data.draft=Object.assign({},data.draft||{},{rules:(Array.isArray(data.rules)?data.rules:[]).map(function(rule){return Object.assign({},rule)})});
  data.draft_stale=false;
  rulesPageState.dirty=true;
  renderRulesPage();
  rulesMarkDirty();
}
function renderRulesPage(){
  ...
  html+='<div class="rules-actions">'+
    ...
    '</div>';
  html+='<p class="rules-foot-note">保存草稿只是存起来，助手看不到；发布之后才会在下一次缓存边界生效。</p>';
...
function chatMergeActiveWorldbookDraft(cfg){
  cfg=cfg&&typeof cfg==='object'?cfg:chatLoadConfig();
  cfg.worldbooks=chatNormalizeWorldbooks(cfg.worldbooks);
  if(!chatWorldbookActiveId)return cfg;
  ...
  var enabled=document.getElementById('chat-worldbook-enabled');
...
function chatRenderWorldbooks(cfg){
  ...
  if(count)count.textContent=enabled+' 个启用（共 '+books.length+' 条）';
  // 以前这里是一条一行的按钮列表：维护到 100 条整页就长得没法用了。
  // 改成一个下拉框，无论多少条都只占一行；下面那一行是「新增世界书」。
  // onchange 仍然走 chatSelectWorldbook，切换前把表单里的改动回写进上一条那套逻辑不变。
  list.innerHTML=books.length?(
    '<div class="chat-worldbook-select-row">'+
      '<label for="chat-worldbook-select">选择</label>'+
      ...
      '</select>'+
    '</div>'
  ):'<div class="chat-worldbook-empty">还没有世界书，点下面的「新增世界书」开一条。</div>';
...
async function chatSaveWorldbook(){
  ...
  var nameEl=document.getElementById('chat-worldbook-name');
  var enabledEl=document.getElementById('chat-worldbook-enabled');
  var priorityEl=document.getElementById('chat-worldbook-priority');
...
function chatDebugCacheDiagKey(record){
  if(!record||record.event!=='debug')return '';
  var data=record.data&&typeof record.data==='object'?record.data:{};
  if(!data.cache_anchors&&!data.canonical_changes)return '';
  if(chatDebugFactRecallSignature(record))return '';
  var id=String(data.debug_id||'').trim();
  return id?('cache-diag:'+id):'';
}
function chatDebugMergeCacheDiag(base,next){
  var merged={};
  Object.keys(base||{}).forEach(function(key){merged[key]=base[key]});
  var baseData=(base&&base.data&&typeof base.data==='object')?base.data:{};
  var nextData=(next&&next.data&&typeof next.data==='object')?next.data:{};
  var data={};
  ...
  merged.data=data;
  merged.ts=Math.max(Number((base&&base.ts)||0),Number((next&&next.ts)||0))||merged.ts;
  return merged;
}
function chatDebugNormalizeRecords(list){
  var out=[],factIndexes={},cacheIndexes={};
  (Array.isArray(list)?list:[]).forEach(function(record){
    if(!record||typeof record!=='object')return;
    var cacheKey=chatDebugCacheDiagKey(record);
    if(cacheKey){
      var cacheIndex=cacheIndexes[cacheKey];
      if(cacheIndex===undefined){
        cacheIndexes[cacheKey]=out.length;
        out.push(record);
        return;
      }
      out[cacheIndex]=chatDebugMergeCacheDiag(out[cacheIndex],record);
      return;
    }
...
var CHAT_DEBUG_TOPICS=[
  {key:'request',label:'请求与传输'},
  {key:'cache',label:'缓存'},
  {key:'recall',label:'召回与 Fact'},
  {key:'timing',label:'计时'},
  {key:'usage',label:'用量与计费'},
  {key:'trim',label:'截断与总结'},
  {key:'error',label:'错误'},
  {key:'other',label:'其他'}
];
function chatDebugTopicLabel(topic){
  for(var i=0;i<CHAT_DEBUG_TOPICS.length;i++){if(CHAT_DEBUG_TOPICS[i].key===topic)return CHAT_DEBUG_TOPICS[i].label}
  return '其他';
}
function chatDebugTopicRank(topic){
  for(var i=0;i<CHAT_DEBUG_TOPICS.length;i++){if(CHAT_DEBUG_TOPICS[i].key===topic)return i}
  return CHAT_DEBUG_TOPICS.length;
}
function chatDebugRecordTopic(record,text){
  var ev=record&&record.event?String(record.event):'';
  var data=(record&&record.data&&typeof record.data==='object')?record.data:{};
  text=String(text||'');
  if(ev==='error')return 'error';
  if(ev==='meta'||ev==='tool')return 'request';
  ...
  if(ev==='debug'){
    ...
    if(data.fact_stats_queued||data.recall_error||data.recall_query||data.memory_chars!==undefined)return 'recall';
  }
  if(text.indexOf('截断')>=0||text.indexOf('措辞偏好')>=0||text.indexOf('总结')>=0)return 'trim';
  if(text.indexOf('缓存')>=0)return 'cache';
  if(text.indexOf('Fact')>=0||text.indexOf('召回')>=0)return 'recall';
  return 'other';
}
function chatDebugRoundOpener(record){
  if(!record)return false;
  if(record.event==='meta')return true;
  return record.event==='debug'&&record.data&&record.data.mode==='new_session';
}
function chatDebugGroupedIndexes(list){
  // 调试台是流式日志，到达顺序天生打散：缓存诊断被意图改写和 Fact 召回顶开，
  // 三条计时也各来各的。这里按「一轮请求」切段，段内按主题稳定排序，
  // 让同类紧挨在一起；每条记录自己的时间戳仍然如实显示。
  list=Array.isArray(list)?list:[];
  var segments=[],current=null;
  list.forEach(function(record,index){
    if(!current||chatDebugRoundOpener(record)){current=[];segments.push(current)}
    current.push(index);
  });
  var out=[];
  segments.forEach(function(indexes){
    indexes.map(function(index){
      var record=list[index];
      var text='';
      try{text=chatDebugLine(record)}catch(e){text=''}
      return {index:index,topic:chatDebugRecordTopic(record,text)};
    }).map(function(item,order){
      item.order=order;
      return item;
    }).sort(function(a,b){
      var diff=chatDebugTopicRank(a.topic)-chatDebugTopicRank(b.topic);
      return diff!==0?diff:(a.order-b.order);
    }).forEach(function(item){out.push(item)});
  });
  return out;
}
...
function chatRenderDebugRecords(){
  ...
  var html='',lastTopic='';
  chatDebugGroupedIndexes(chatDebugRecords).forEach(function(item){
    if(item.topic!==lastTopic){
      html+='<div class="chat-debug-topic"><span>'+esc(chatDebugTopicLabel(item.topic))+'</span></div>';
...
function chatScrollDebugBottom(){
  ...
  scroll();
  if(chatDebugScrollRaf&&typeof cancelAnimationFrame==='function')cancelAnimationFrame(chatDebugScrollRaf);
  if(typeof requestAnimationFrame==='function'){
    chatDebugScrollRaf=requestAnimationFrame(function(){
      scroll();
      chatDebugScrollRaf=requestAnimationFrame(scroll);
    });
  }
  clearTimeout(chatDebugScrollTimer);
  chatDebugScrollTimer=setTimeout(scroll,260);
}
function chatFormatRecallDiag(data){
  data=data||{};
  var diag=(data.recall_diag&&typeof data.recall_diag==='object')?data.recall_diag:null;
  if(!diag)return '🧠 Fact 召回｜本轮注入 '+(data.memory_chars||0)+' 字';
  var recallMode=diag.recall_mode||data.recall_mode||'fact_only';
  ...
  if(!hasSources){
    var skipped=diag.low_information_skipped?'｜低信息消息，已跳过查询':'';
    return '🧠 Fact 召回｜本轮注入 '+(data.memory_chars||diag.memory_chars||0)+' 字'+skipped;
  }
  ...
  var factStore=(sources.fact_store&&typeof sources.fact_store==='object')?sources.fact_store:{};
  var factVectors=(sources.fact_vectors&&typeof sources.fact_vectors==='object')?sources.fact_vectors:{};
  var phaseText=phases.filter(function(p){return p&&typeof p==='object'}).map(function(p){
    return String(p.name||'未知阶段')+' '+sec(p.seconds);
  }).join('｜')||'无';
  var lines=['🧠 Fact 召回｜'+modeMeta.label+'｜本轮注入 '+(data.memory_chars||diag.memory_chars||0)+' 字｜总耗时 '+sec(diag.total_seconds)];
  ...
  var seenGroups={};
  var evidenceText=Object.keys(selectedStats).map(function(key){
    var stat=selectedStats[key]||{};
    var group=String(stat.version_group_id||key);
  ...
  var pathB=String(diag.fact_recall_path||'').toLowerCase()==='b';
  var rewriteOutput=(diag.rewrite_output&&typeof diag.rewrite_output==='object')?diag.rewrite_output:{};
  var pathBRewrite=pathB?JSON.stringify(rewriteOutput):'';
  var exemptionEvents=Array.isArray(diag.exemption_events)?diag.exemption_events:[];
  var fallbackRecovered=Array.isArray(diag.fallback_recovered)?diag.fallback_recovered:[];
  var refine=(diag.refine&&typeof diag.refine==='object')?diag.refine:{};
  var pathBExemptions=exemptionEvents.map(function(item){
    return (item.fact_id||'-')+' '+Number(item.score||0).toFixed(3)+'｜'+(item.reason||'-')+'｜'+((item.waived_rules||[]).join(',')||'-');
  }).join('；')||'无';
  var pathBFallback=fallbackRecovered.map(function(item){
    return (item.fact_id||'-')+' '+Number(item.score||0).toFixed(3)+'｜从 '+(item.recovered_from||'-')+' 捞回';
  }).join('；')||'未触发';
  var refineCandidates=Array.isArray(refine.candidates)?refine.candidates:[];
  var pathBRefine=refineCandidates.map(function(item){
    return '#'+(item.number||'-')+' '+(item.fact_id||'-')+' '+Number(item.score||0).toFixed(3);
  }).join('；')||'无候选';
  pathBRefine+='｜返回 '+((refine.returned_ids||[]).join(',')||'无')+'｜'+(refine.status||'-');
  ...
  lines.push('🧪 候选逐条诊断｜\n'+candidateText);
  if(pathB){
    lines.push('🅱️ B 改写完整 JSON｜'+pathBRewrite);
    lines.push('🪶 B 过滤豁免｜'+pathBExemptions);
    lines.push('🛟 B 强制保底｜'+pathBFallback);
    lines.push('🧹 B 精筛｜'+pathBRefine);
  }
...
function chatFormatFactStatsLine(data){
  data=data&&typeof data==='object'?data:{};
...
var CHAT_DIAG_TOKEN_LABELS={
  body:'正文/其他文本',native_thinking:'原生思考正文',pseudo_thinking:'伪思考',
  signature:'原生思考签名',redacted_thinking:'加密思考',recall:'召回',
  backend_switch:'后端切换通知',worldbook:'世界书',memory_pack:'固定记忆包',
  daily_digest:'当日截断总结',session_anchor:'会话首条锚点',speech_preferences:'措辞偏好',
  thinking_instruction:'思考提示词',tool_call:'工具调用参数',tool_result:'工具返回结果',
  tools_schema:'工具定义',image:'图片',document:'文档',unknown_block:'其他非文本块'
};
var CHAT_DIAG_HIDDEN_PARTS={
  current_time:true,time_gap:true,context_wrapper:true,reply_target:true,
  time_reply_hint:true,time_protocol:true
};
var CHAT_DIAG_REQUEST_AREAS={
  system:'系统区',history:'历史区',current:'当前用户消息',
  continuation:'本轮工具续传',tools:'工具定义'
};
function chatDiagTokenText(value,fallback){
  if(value===null||value===undefined||value==='')return fallback||'未记录';
  var n=Number(value);
  return isFinite(n)?dsNum(Math.round(n))+' token':(fallback||'未记录');
}
function chatDiagSnapshotToken(row){
  if(!row)return '未记录';
  if(row.estimated_tokens===null)return '—';
  return chatDiagTokenText(row.estimated_tokens);
}
function chatDiagActualToken(value){
  return value===null||value===undefined?'未返回':chatDiagTokenText(value);
}
function chatDiagNumber(value){
  return value===null||value===undefined?'未返回':String(value);
}
function chatDiagPolicy(value){
  return value===false?'关闭':value===true?'保留':'未记录';
}
function chatDiagPartLabel(kind){
  return CHAT_DIAG_TOKEN_LABELS[kind]||kind;
}
function chatDiagStatus(data){
  return ({complete:'已完成',failed:'失败',upstream_returned:'已收到上游返回'}[data&&data.diagnostic_status]||'等待上游用量');
}
function chatDiagUsageLine(prefix,usage){
  var u=usage||{};
  return prefix+'输入总计 '+chatDiagActualToken(u.input_total)+' = 普通输入 '+chatDiagActualToken(u.input_uncached)+
    ' + 缓存读取 '+chatDiagActualToken(u.cache_read)+' + 缓存创建 '+chatDiagActualToken(u.cache_create)+
    '\n  输出合计 '+chatDiagActualToken(u.output)+'｜原生思考（输出子集）'+chatDiagActualToken(u.thinking)+
    '｜缓存命中率 '+(u.cache_hit_percent===null||u.cache_hit_percent===undefined?'未返回':u.cache_hit_percent+'%')+
    '\n  缓存创建分档：5m '+chatDiagActualToken(u.cache_create_5m)+' / 1h '+chatDiagActualToken(u.cache_create_1h);
}
function chatDiagShouldHidePart(kind){
  return !!CHAT_DIAG_HIDDEN_PARTS[kind];
}
function chatDiagCacheSummary(data){
  data=data&&typeof data==='object'?data:{};
  var meta=chatCacheStrategyMeta(data.cache_strategy||'single_5m');
  var compare=data.cache_fingerprint&&data.cache_fingerprint.compare_previous;
  var compareText=compare&&typeof compare==='object'
    ?(compare.common_prefix_token_estimate!==undefined
      ?'可复用前缀约 '+chatDiagTokenText(compare.common_prefix_token_estimate)+'（'+(compare.common_prefix_segments||0)+' 段）'
      :'上一轮前缀暂无比较')
    :'上一轮前缀暂无比较';
  return meta.label+'｜TTL '+(data.prompt_cache_ttl||'上游决定')+'｜'+compareText;
}
function chatFormatRequestAccounting(data){
  data=data&&typeof data==='object'?data:{};
  var request=data.request_details||{};
  var lines=['🧊 缓存诊断｜请求 '+(data.debug_id||'-')+'｜'+(request.model||'-')+'｜'+chatDiagStatus(data)];
  lines.push('📊 实际用量（上游 usage）');
  if(data.diagnostic_usage_total)lines.push(chatDiagUsageLine('  整次聊天累计：',data.diagnostic_usage_total));
  else lines.push('  尚无完整累计用量；未返回的字段不会记作 0。');
  lines.push('  缓存创建含新尾部写入及重建；创建量大于 0 不代表整段缓存失效。');
  lines.push('📋 历史保留设置（仅控制下一轮发送历史）');
  [
    ['原生思考','retain_native_thinking_history'],['伪思考','retain_pseudo_thinking_history'],
    ['召回','retain_recall_history'],['后端切换通知','retain_backend_switch_history']
  ].forEach(function(entry){lines.push('  '+entry[0]+'：'+chatDiagPolicy(data[entry[1]]));});
  lines.push('  本轮思考模式：'+(request.thinking_type||'off')+'｜固定预算 '+chatDiagNumber(request.thinking_budget)+
    '｜最大输出 '+chatDiagNumber(request.max_output_tokens)+' token');
  var cleanup=data.history_cleanup;
  if(cleanup){
    lines.push('🧹 清理账单（新用户轮次开始时执行一次）');
    lines.push('  来源 '+(data.history_source||'-')+'｜读取 '+(cleanup.before&&cleanup.before.messages||0)+' 条 → 清理后 '+(cleanup.after&&cleanup.after.messages||0)+' 条');
    lines.push('  清理前 '+chatDiagSnapshotToken(cleanup.before)+' → 清理后 '+chatDiagSnapshotToken(cleanup.after)+'｜移除 '+chatDiagSnapshotToken(cleanup.removed));
    lines.push('  网关上下文处理：'+(cleanup.strip_all_context?'整块清理（模式切换/召回关闭/策略或空闲到期）':'按各历史开关过滤')+
      '｜原生思考清理 '+chatDiagNumber((cleanup.native||{}).messages)+' 条消息 / '+chatDiagNumber((cleanup.native||{}).blocks)+' 块'+
      '｜伪思考清理 '+chatDiagNumber((cleanup.pseudo||{}).messages)+' 条消息 / '+chatDiagNumber((cleanup.pseudo||{}).spans)+' 段');
    var keys=['native_thinking','pseudo_thinking','recall','backend_switch','signature','redacted_thinking'];
    Object.keys((cleanup.removed||{}).parts||{}).forEach(function(k){
      if(keys.indexOf(k)<0&&!chatDiagShouldHidePart(k))keys.push(k);
    });
    keys.forEach(function(k){
      var before=((cleanup.before||{}).parts||{})[k];
      var after=((cleanup.after||{}).parts||{})[k];
      var removed=((cleanup.removed||{}).parts||{})[k];
      lines.push('  '+chatDiagPartLabel(k)+'：'+chatDiagSnapshotToken(before)+' → '+chatDiagSnapshotToken(after)+'｜移除 '+chatDiagSnapshotToken(removed));
    });
  }else lines.push('🧹 清理账单：旧日志未记录。');
  var describeRequest=function(snapshot,prefix){
    snapshot=snapshot||{};
    lines.push(prefix+chatDiagSnapshotToken(snapshot)+'｜'+(snapshot.messages||0)+' 条消息');
    Object.keys(CHAT_DIAG_REQUEST_AREAS).forEach(function(key){
      var area=(snapshot.areas||{})[key];
      if(!area)return;
      lines.push('  '+CHAT_DIAG_REQUEST_AREAS[key]+'合计：'+chatDiagSnapshotToken(area));
      Object.keys(area.parts||{}).forEach(function(kind){
        if(chatDiagShouldHidePart(kind))return;
        lines.push('    '+chatDiagPartLabel(kind)+'：'+chatDiagSnapshotToken(area.parts[kind]));
      });
    });
  };
  lines.push('🧾 输入 token 明细（网关组装后的请求，各区域互不重复）');
  describeRequest(request,'  请求合计：');
  var latest=(request.areas||{}).current||{},history=(request.areas||{}).history||{};
  lines.push('📥 本轮动态注入 / 最终历史残留（仅保留可发送内容）');
  ['recall','backend_switch','native_thinking','pseudo_thinking'].forEach(function(kind){
    lines.push('  '+chatDiagPartLabel(kind)+'：本轮 '+chatDiagSnapshotToken((latest.parts||{})[kind])+'｜最终历史 '+chatDiagSnapshotToken((history.parts||{})[kind]));
  });
  ...
  rounds.forEach(function(round){
    ...
    lines.push(chatDiagUsageLine('  ',round.usage));
    lines.push('  输出 token 明细（思考已包含在输出合计内）：');
    Object.keys((round.output||{}).parts||{}).forEach(function(kind){
      if(chatDiagShouldHidePart(kind))return;
      lines.push('    '+chatDiagPartLabel(kind)+'：'+chatDiagSnapshotToken(round.output.parts[kind]));
    });
  ...
  (data.diagnostic_failed_attempts||[]).forEach(function(attempt,index){
    ...
    lines.push(chatDiagUsageLine('  已收到的用量：',attempt.usage));
  ...
  lines.push('召回路径：'+(data.fact_recall_mode?chatFactRecallModeMeta(data.fact_recall_mode).label:'未记录')+
    '｜召回状态：'+(data.recall_enabled===false?'关闭':(data.gateway_context_injected?'已注入':'无注入')));
...
function chatFormatRequestAccountingHtml(data){
  data=data&&typeof data==='object'?data:{};
  var request=data.request_details||{},rows=[],usage=data.diagnostic_usage_total||{};
  function add(area,item,token,note){rows.push({area:area,item:item,token:token,note:note||''});}
  add('实际用量','输入总计',chatDiagActualToken(usage.input_total),'普通 '+chatDiagActualToken(usage.input_uncached)+' / 读取 '+chatDiagActualToken(usage.cache_read)+' / 创建 '+chatDiagActualToken(usage.cache_create));
  add('实际用量','输出合计',chatDiagActualToken(usage.output),'原生思考（输出子集） '+chatDiagActualToken(usage.thinking));
  add('实际用量','缓存命中率',usage.cache_hit_percent===null||usage.cache_hit_percent===undefined?'未返回':usage.cache_hit_percent+'%','5m 创建 '+chatDiagActualToken(usage.cache_create_5m)+' / 1h 创建 '+chatDiagActualToken(usage.cache_create_1h));
  Object.keys(CHAT_DIAG_REQUEST_AREAS).forEach(function(key){
    var area=(request.areas||{})[key];
    if(!area)return;
    add('输入 · '+CHAT_DIAG_REQUEST_AREAS[key],'区域合计',chatDiagSnapshotToken(area),'请求组装区');
    Object.keys(area.parts||{}).forEach(function(kind){
      if(chatDiagShouldHidePart(kind))return;
      add('输入 · '+CHAT_DIAG_REQUEST_AREAS[key],'↳ '+chatDiagPartLabel(kind),chatDiagSnapshotToken(area.parts[kind]),'分项');
    });
  });
  var cleanup=data.history_cleanup;
  if(cleanup){
    add('历史清理','清理前',chatDiagSnapshotToken(cleanup.before),'读取 '+((cleanup.before||{}).messages||0)+' 条消息');
    add('历史清理','清理后',chatDiagSnapshotToken(cleanup.after),'保留 '+((cleanup.after||{}).messages||0)+' 条消息');
    add('历史清理','移除',chatDiagSnapshotToken(cleanup.removed),'本轮历史预处理');
    ['native_thinking','pseudo_thinking','recall','backend_switch','signature','redacted_thinking'].forEach(function(kind){
      var removed=((cleanup.removed||{}).parts||{})[kind];
      if(removed)add('历史清理','↳ 移除 '+chatDiagPartLabel(kind),chatDiagSnapshotToken(removed),'分项');
    });
  }
  var fp=data.cache_fingerprint&&typeof data.cache_fingerprint==='object'?data.cache_fingerprint:{};
  ...
  add('缓存','上一轮可复用断点',reusable.length+' 个',reusable.length?'可继续读取':'本轮可能重建');
  add('缓存','清理状态',data.strip_old_recall?'已清理旧动态历史':'按保留策略处理','canonical '+(data.canonical_sessions_cleared||0));
  (Array.isArray(data.diagnostic_rounds)?data.diagnostic_rounds:[]).forEach(function(round){
    var u=round.usage||{};
    add('上游第 '+round.round+' 次','输入 / 输出',chatDiagActualToken(u.input_total)+' / '+chatDiagActualToken(u.output),round.ok?'成功':'失败');
  });
  var status=chatDiagStatus(data);
  var meta=chatCacheStrategyMeta(data.cache_strategy||'single_5m');
  var chips=[
    ['请求',data.debug_id||'-'],['模型',request.model||'-'],['状态',status],
    ['策略',meta.label],['TTL',data.prompt_cache_ttl||'上游决定'],
    ['消息',(request.messages||0)+' 条']
  ];
  var chipHtml=chips.map(function(pair){
    return '<span class="chat-debug-sheet-chip"><b>'+esc(pair[0])+'</b><span>'+esc(String(pair[1]))+'</span></span>';
  }).join('');
  return '<div class="chat-debug-sheet" data-debug-sheet="cache"><div class="chat-debug-sheet-head">'+
    '<div><strong>🧊 缓存诊断</strong><small>只显示 token 与缓存状态</small></div>'+
    '<div class="chat-debug-sheet-chip-row">'+chipHtml+'</div></div>'+
...
function chatFormatDebug(ev,data){
  ...
  function fingerprintZh(fp){
    ...
    var commonText=cmp.common_prefix_bytes!==undefined
      ?('｜相同前缀 ~'+(cmp.common_prefix_token_estimate||0)+' token（'+(cmp.common_prefix_segments||0)+' 段）')
      :'';
    ...
    var sizes=details.map(function(x){
      return String(x.label||'-')+':~'+(x.prefix_token_estimate||0)+' token';
    });
    var sizeText=sizes.length?'｜断点累计 '+sizes.slice(0,4).join('，'):'';
    var breakpointText=details.length
      ?details.map(function(x){return String(x.label||'-')+(x.ttl?'@'+x.ttl:'')}).join('，')
      :bps.map(function(x){return String(x).replace(/@\d+B/g,'')}).join('，');
    return '｜'+status+commonText+firstChangeText+reusableText+stableText+changeText+'｜请求 '+(fp.request_hash||'-')+'（哈希不含断点标记）｜断点 '+(breakpointText||'-')+sizeText;
  }
  if(ev==='meta'){
    ...
    var windowText=data.window_history_supplied?('｜窗口历史：'+(data.window_history_messages||0)+' 条'):'';
     var thinkingMode=data.thinking_mode||(
       data.native_thinking_enabled?'native':(data.ck_thinking_enabled?'compat':'off')
     );
    ...
    return '🧭 请求信息｜会话：'+(data.session_id||'-')+'｜模型：'+(data.model||'-')+backendText+'｜历史来源：'+sourceText+'｜历史条数：'+(data.history_messages||0)+windowText+'｜首条锚点：'+(data.session_anchor_chars||0)+' 字｜世界书：'+(data.worldbook_chars||0)+' 字'+dailyDigestText+thinkingText+injectionText+targetText+mcpText;
  }
  if(ev==='memory'){
  ...
  if(ev==='speech_preference_prepare'){
    ...
    if(data.ok===false)return '⚠️ 措辞偏好提取失败｜本批 '+(data.messages||0)+' 条｜丢弃 '+(data.dropped_rounds||0)+' 轮｜'+(data.duration_ms||0)+'ms｜'+(data.stopped?'已被停止｜':'')+(data.error||'未知原因');
    return '🗣 措辞偏好提取｜本批 '+(data.messages||0)+' 条｜丢弃 '+(data.dropped_rounds||0)+' 轮｜版本 '+(data.speech_preference_revision||'-')+'（待生效 '+(data.speech_preference_pending_revision||'-')+' / 已生效 '+(data.speech_preference_applied_revision||'-')+'）｜'+(data.duration_ms||0)+'ms';
  }
  if(ev==='speech_preference_prepare_failed_nonblocking'){
    return '⚠️ 措辞偏好提取失败（不阻塞截断）｜保留旧规则｜触发：'+(data.trim_trigger||'-')+'｜计划丢弃 '+(data.planned_dropped||0)+' 轮｜进重试队列 '+(data.queued_rows||0)+' 条｜'+(data.error||'未知原因');
  }
  ...
  if(ev==='debug'){
    ...
    if(data.cache_anchors||data.canonical_changes){
      ...
      var breakdownText=data.request_details?'':'｜请求拆分 token：旧日志未记录';
    ...
    if(data.fact_stats_queued){
      return chatFormatFactStatsLine(data);
    }
...
function chatAttachAssistantTiming(msg,userSentTs,replyTs){
  if(!msg||msg.role!=='assistant')return msg;
  replyTs=Number(replyTs||msg.ts||Date.now())||Date.now();
  msg.ts=replyTs;
  userSentTs=Number(userSentTs||0)||0;
  if(userSentTs){
    msg.userSentTs=userSentTs;
    msg.firstReplyTs=replyTs;
    msg.waitMs=Math.max(0,replyTs-userSentTs);
  }
  return msg;
}
function chatUpdateStreamingTiming(anchorEl,userSentTs,replyTs){
  if(!anchorEl||!anchorEl.parentNode)return;
  var meta=anchorEl.parentNode.querySelector('.chat-msg-time');
  if(!meta)return;
  var msg=chatAttachAssistantTiming({role:'assistant',ts:replyTs},userSentTs,replyTs);
  var html=chatMessageTimingHtml(msg,'assistant');
...
function chatAssistantWaitLabel(m){
  ...
  return '等待 '+Math.max(0,Math.round(waitMs/1000))+'s';
}
function chatUpdateRuntime(cfg,usage){
  // 顶栏那三枚状态条（模型 / 网关 session / 召回状态）已删除：它被两份 CSS 用 !important
  // 永久隐藏，而这三项在「网关」「记忆与缓存」面板和调试台里都能看到，属于重复信息。
  // 这个函数保留下来只做一件仍然有用的事：把"要不要显示缓存勾号 / 单条价格"
  // 这两个判断同步成 body 上的类，给 CSS 留一道兜底。
  chatApplyDisplayGateClasses();
}
function chatRenderTrimState(cfg){
  ...
  var speechHint=chatSpeechPreferencesEnabledForTrim();
  if(expired){
    if(count>trim.keep){
      next.textContent='缓存已连续 1h 未读取或创建：下一次发送前'+(speechHint?'先审阅措辞偏好，再':'')+'保留最近 '+trim.keep+' 个真实轮次。 '+roundLimitText;
    }else{
      next.textContent='缓存已连续 1h 未读取或创建：下一次发送前'+(speechHint?'更新措辞偏好；':'')+'真实轮次不足 '+trim.keep+'，不删除内容。 '+roundLimitText;
    }
  }else{
...
function chatNormalizeFolder(folder,index){
  folder=folder&&typeof folder==='object'?folder:{};
  return {
    id:String(folder.id||('f_'+Date.now().toString(36)+'_'+String(index||0))),
    name:String(folder.name||'未命名文件夹').trim().slice(0,40)||'未命名文件夹',
    color:String(folder.color||''),
    collapsed:folder.collapsed===true,
    order:isFinite(Number(folder.order))?Number(folder.order):Number(index||0),
    created:Number(folder.created||Date.now())||Date.now()
  };
}
function chatLoadFolders(){
  try{
    var raw=JSON.parse(localStorage.getItem(CHAT_FOLDERS_KEY)||'[]');
    chatFolders=Array.isArray(raw)?raw.map(chatNormalizeFolder):[];
  }catch(e){chatFolders=[]}
  chatFolders.sort(function(a,b){return a.order-b.order||a.created-b.created});
}
function chatSaveFolders(){
  try{localStorage.setItem(CHAT_FOLDERS_KEY,JSON.stringify(chatFolders.map(chatNormalizeFolder)))}catch(e){}
}
function chatFolderById(id){
  return chatFolders.find(function(folder){return folder.id===String(id||'')})||null;
}
function chatFolderId(){return 'f_'+Date.now().toString(36)+'_'+Math.random().toString(36).slice(2,7)}
async function chatCreateFolder(name){
  var value=name;
  if(value===undefined)value=await ckPromptDialog('新建文件夹','',{placeholder:'文件夹名称',confirmText:'创建'});
  if(value===null)return null;
  value=String(value||'').trim();
  if(!value){toast('请输入文件夹名称');return null}
  var folder=chatNormalizeFolder({id:chatFolderId(),name:value,order:chatFolders.length,created:Date.now()},chatFolders.length);
  chatFolders.push(folder);
  chatSaveFolders();
  chatRenderSessions();
  return folder;
}
async function chatRenameFolder(id){
  var folder=chatFolderById(id);if(!folder)return;
  var value=await ckPromptDialog('重命名文件夹',folder.name,{placeholder:'文件夹名称',confirmText:'保存'});
  if(value===null)return;
  value=String(value||'').trim();
  if(!value){toast('文件夹名称不能为空');return}
  folder.name=value.slice(0,40);
  chatSaveFolders();chatRenderSessions();
}
async function chatDeleteFolder(id){
  var folder=chatFolderById(id);if(!folder)return;
  var count=chatSessions.filter(function(session){return session.folderId===id}).length;
  var confirmed=await ckConfirmDialog('删除“'+folder.name+'”文件夹？其中 '+count+' 个对话会移到未分组，不会被删除。',{title:'删除文件夹',confirmText:'删除',danger:true});
  if(!confirmed)return;
  chatSessions.forEach(function(session){if(session.folderId===id)session.folderId=''});
  chatFolders=chatFolders.filter(function(item){return item.id!==id});
  chatFolders.forEach(function(item,index){item.order=index});
  chatSaveFolders();chatSaveSessions();chatRenderSessions();
}
...
function chatRenderSessions(){
  ...
  var validFolderIds={};chatFolders.forEach(function(folder){validFolderIds[folder.id]=true});
  var filtered=chatSessions.filter(function(session){
    if(!chatSessionSearch)return true;
    return [session.title,chatSessionPreview(session),chatSessionMeta(session),(chatFolderById(session.folderId)||{}).name].join(' ').toLowerCase().indexOf(chatSessionSearch)>=0;
  });
  var rows='';
  if(chatSessionSearch){
    rows=filtered.map(function(session){var folder=chatFolderById(session.folderId);return chatSessionRowHtml(session,folder?folder.name:'未分组')}).join('');
    if(!rows)rows='<div class="chat-session-search-empty">没有匹配的对话</div>';
  }else{
...
async function chatDeleteSession(id,event){
  if(event){
...
function chatInsertRenderedMessageRow(box,html,index){
  if(!box)return;
  var rows=Array.prototype.slice.call(box.querySelectorAll('.chat-msg-row[data-chat-index]'));
  var next=rows.find(function(row){return Number(row.getAttribute('data-chat-index'))>index});
...
function chatUpdateMessageRowOnly(index){
  ...
  if(chatIsAssistantRevealPending(message)){
    if(row)row.remove();
    return null;
  }
  var html=chatRenderMessageRow(message,index);
  if(row)row.outerHTML=html;
  else chatInsertRenderedMessageRow(box,html,index);
  return box.querySelector('.chat-bubble[data-i="'+String(index)+'"]');
}
function chatYieldAfterVisualFeedback(){
  return new Promise(function(resolve){
    if(typeof requestAnimationFrame!=='function'){setTimeout(resolve,0);return}
    requestAnimationFrame(function(){setTimeout(resolve,0)});
  });
}
function chatDeferAfterSendPaint(fn){
...
function chatPlanAutoTrimForPendingBatch(cfg,submittedPending,opts){
  opts=opts||{};
  ...
  var session=chatCurrentSession();
  var localPlan=CHAT_HISTORY_TOOLS
    ?CHAT_HISTORY_TOOLS.trimLocalTurns(working,trim.keep,{excluded:selected})
    :{before:chatAutoTrimRoundCount(working),after:chatAutoTrimRoundCount(working),dropped:0,keptMessages:working.slice(),droppedMessages:[]};
  var transportMessages=Array.isArray(session&&session.transportMessages)?session.transportMessages:[];
  var transportPlan=CHAT_HISTORY_TOOLS
    ?CHAT_HISTORY_TOOLS.trimTransportTurns(transportMessages,trim.keep)
    :{before:chatTransportRoundCount(transportMessages),after:chatTransportRoundCount(transportMessages),dropped:0,keptMessages:transportMessages.slice(),droppedMessages:[]};
  var canonicalTransport=transportPlan.before>0;
  var historyRounds=canonicalTransport?transportPlan.before:localPlan.before;
  ...
  var base={
    ...
    roundLimitBoundary:roundLimitBoundary,
    roundLimit:trim.roundLimit,
    ...
    keptMessages:working,droppedMessages:[],
    keptTransportMessages:transportMessages.slice(),
    droppedTransportMessages:[],
    canonicalTransport:canonicalTransport,
    transportBefore:transportPlan.before,transportAfter:transportPlan.before,transportDropped:0,
    localBefore:localPlan.before,localAfter:localPlan.before,localDropped:0,
  ...
  var drop=shouldTrim?(canonicalTransport?transportPlan.dropped:localPlan.dropped):0;
  var after=Math.max(0,historyRounds-drop);
  var preparedThrough=Number(session&&session.speechPreferencePreparedThroughTs||0)||0;
  var preferenceMessages=pendingBoundary?[]:working.filter(function(message){
    if(!message||typeof message!=='object')return false;
    if(message.role!=='user'&&message.role!=='pending_user'&&message.role!=='assistant')return false;
    if(message.role==='assistant'&&message.stopped===true)return false;
    var ts=Number(message.ts||0)||0;
    return !preparedThrough||!ts||ts>preparedThrough;
  });
  var preferenceThroughTs=preferenceMessages.reduce(function(latest,message){
    return message&&(message.role==='user'||message.role==='pending_user')
      ?Math.max(latest,Number(message.ts||0)||0)
      :latest;
  },preparedThrough);
  var requiredPreferenceThroughTs=droppedMessages.reduce(function(latest,message){
    return message&&(message.role==='user'||message.role==='pending_user')
      ?Math.max(latest,Number(message.ts||0)||0)
      :latest;
  },preparedThrough);
  return {
    ...
    cacheAgeMs:cacheAgeMs,
    cacheReferenceTs:cacheReferenceTs,
    cacheReferenceSource:base.cacheReferenceSource,
    lastActivityTs:lastActivityTs,
    cacheNoticePresent:base.cacheNoticePresent,
    trimmed:shouldTrim&&drop>0,
    before:historyRounds,
    after:after,
    dropped:drop,
    historyBefore:historyRounds,
    historyAfter:after,
    keep:trim.keep,
    droppedMessages:droppedMessages,
    keptMessages:keptMessages,
    keptTransportMessages:keptTransportMessages,
    droppedTransportMessages:shouldTrim&&canonicalTransport?transportPlan.droppedMessages:[],
    canonicalTransport:canonicalTransport,
    transportBefore:transportPlan.before,
    transportAfter:shouldTrim&&canonicalTransport?transportPlan.after:transportPlan.before,
    transportDropped:shouldTrim&&canonicalTransport?transportPlan.dropped:0,
    localBefore:localPlan.before,
    localAfter:shouldTrim?localPlan.after:localPlan.before,
    localDropped:shouldTrim?localPlan.dropped:0,
    deferred:deferred,
    editingMessage:base.editingMessage,
    preferenceMessages:preferenceMessages,
    preferenceThroughTs:preferenceThroughTs,
    requiredPreferenceThroughTs:requiredPreferenceThroughTs,
    preparedThroughTs:preparedThrough,
    forceCacheRebuild:pendingBoundary||drop>0,
  };
}
function chatSpeechPreferenceMessageId(message,index){
  if(!message||typeof message!=='object')return '';
  if(!message.messageId){
    message.messageId='msg-'+String(Number(message.ts||0)||Date.now())+'-'+String(index||0)+'-'+Math.random().toString(36).slice(2,7);
  }
...
function chatSpeechPreferenceQueueCommit(session,prepared){
  if(!session)return;
  if(prepared&&prepared.skipSpeech===true)return;
...
function chatSpeechPreferencePrepareBatch(list,queued){
  ...
  list.forEach(function(message,index){
    if(!message||typeof message!=='object')return;
    if(message.role==='assistant'&&message.stopped!==true&&String(message.text||'').trim()){
      previousAssistant=String(message.text||'').trim().slice(-600);
      return;
    }
  ...
  for(var i=0;i<candidates.length;i++){
    ...
    var size=row.text.length+row.assistant_context.length+100;
    if(messages.length&&chars+size>maxChars)break;
    messages.push(row);
    chars+=size;
  }
  var reviewedThroughTs=messages.reduce(function(latest,row){return Math.max(latest,Number(row.ts||0)||0)},0);
  return {
    messages:messages,
...
function chatSpeechPreferenceResponseMeta(data){
  ...
  return {
    ...
    activationStatus:String(data.speech_preference_activation_status||data.speech_preference_status||data.activation_status||nested.activation_status||nested.status||'').trim(),
    activationError:String(data.speech_preference_activation_error||data.activation_error||nested.activation_error||'').trim(),
...
function chatCaptureSpeechPreferenceRevisions(data,sessionId,opts){
  ...
  if(meta.revision===''&&meta.appliedRevision===''&&!meta.activationApplied&&!meta.activationStatus&&!meta.activationError)return meta;
  ...
  if(opts.allowClear!==false&&pendingId&&activationConfirmed){
    ...
    session.speechPreferencePendingThroughTs=0;
    session.speechPreferencePendingBoundaryReason='';
    session.speechPreferenceRetryAtBoundary=false;
    session.cacheRebuildPending=false;
    changed=true;
  }else if(opts.allowClear!==false&&pendingId&&!meta.activationApplied&&(
    !!meta.activationError||/failed|error|mismatch|pending_retry|no_pending/i.test(meta.activationStatus)
  )){
    // 明确激活失败后，不在普通下一轮继续改 system 前缀。保留候选，
    // 只在下一次 1h 缓存边界或用户手动截断时重试。
    if(session.speechPreferenceRetryAtBoundary!==true){
      session.speechPreferenceRetryAtBoundary=true;
      changed=true;
    }
    if(session.cacheRebuildPending===true){
      session.cacheRebuildPending=false;
      changed=true;
    }
  }
...
function chatSpeechPreferenceTrimEventId(cfg,plan){
  return [
    'speech-boundary',
    String(plan&&plan.trigger||'trim'),
    String(cfg&&cfg.sessionId||'session'),
    Number(plan&&plan.lastActivityTs||0)||0,
    Number(plan&&plan.preferenceThroughTs||0)||0,
    Number(plan&&plan.dropped||0),
  ].join('-');
}
...
async function chatPrepareSpeechPreferencesForTrim(cfg,plan,requestState){
  ...
  chatSetStatus('正在请求网关：缓存重建前更新措辞偏好...');
  ...
  try{
    ...
    var response=await fetch(chatSpeechPreferencePrepareEndpoint(cfg),{
      ...
      body:JSON.stringify({
        ...
        event_id:eventId,
        messages:messages,
        reason:plan&&plan.manual?'trim':'auto_trim',
        activate_on:'trim'
      })
    });
    var data={};
    ...
    var hasBackendReviewed=Object.prototype.hasOwnProperty.call(data,'reviewed_through_ts')||
      Object.prototype.hasOwnProperty.call(data,'speech_preference_reviewed_through_ts');
    var backendReviewed=Number(
      Object.prototype.hasOwnProperty.call(data,'reviewed_through_ts')
        ?data.reviewed_through_ts:data.speech_preference_reviewed_through_ts
    )||0;
    var reviewedThroughTs=hasBackendReviewed?backendReviewed:(batch.reviewedThroughTs||0);
    return {
      ok:true,eventId:eventId,activationId:activationId,
...
function chatDiscardPendingSpeechPreference(session){
  if(!session)return false;
  var changed=false;
  ['speechPreferencePendingActivationId','speechPreferencePendingTrimEventId','speechPreferencePendingBoundaryReason'].forEach(function(key){
    if(session[key]){session[key]='';changed=true}
  });
  if(Number(session.speechPreferencePendingThroughTs||0)){session.speechPreferencePendingThroughTs=0;changed=true}
...
function chatCommitAutoTrimPlan(cfg,plan,prepared){
  ...
  if(prepared&&prepared.activationId){
    ...
    s.speechPreferencePendingBoundaryReason=String(plan.trigger||'trim');
    s.speechPreferenceRetryAtBoundary=false;
    s.speechPreferencePendingThroughTs=Math.max(
      Number(s.speechPreferencePendingThroughTs||0)||0,
      reviewedThroughTs
    );
  }
  ...
  return {
    ...
    historyBefore:plan.historyBefore,
    historyAfter:trimCommitted?plan.historyAfter:plan.historyBefore,
    keep:plan.keep,
    transportCleared:trimCommitted&&!plan.canonicalTransport?oldTransport:0,
    forceCacheRebuild:!!(plan.forceCacheRebuild&&trimCommitted),
    canonicalTransport:!!plan.canonicalTransport,
    transportBefore:plan.transportBefore||0,
    transportAfter:trimCommitted?(plan.transportAfter||0):(plan.transportBefore||0),
    localBefore:plan.localBefore||0,
    localAfter:trimCommitted?(plan.localAfter||0):(plan.localBefore||0),
    speechPreferenceReviewedThroughTs:reviewedThroughTs,
    speechPreferenceReviewComplete:prepared.reviewComplete!==false,
...
async function chatApplyAutoTrimForPendingBatch(cfg,submittedPending,requestState,opts){
  ...
  // 只截断时完全跳过规则管理；规则管理关闭时也不准备、不排队、不发送措辞偏好。
  var skipSpeech=opts&&opts.skipSpeech===true;
  var speechEnabled=skipSpeech?false:await chatEnsureSpeechPreferenceState();
  if(!speechEnabled&&chatDiscardPendingSpeechPreference(chatCurrentSession()))chatSaveSessions();
  skipSpeech=skipSpeech||!speechEnabled;
...
async function chatManualTrimNow(){
  ...
  try{
    var result=await chatApplyAutoTrimForPendingBatch(cfg,[],null,{force:true,trigger:'manual_trim',skipSpeech:true});
...
async function chatMaybeAutoTrimAtIdleBoundary(opts){
  ...
  var cfg=chatLoadConfig();
  var trim=chatAutoTrimConfigFrom(cfg);
  if(!trim.enabled&&!trim.roundLimitEnabled)return;
  var session=chatCurrentSession();
  if(!session)return;
  ...
  try{
    var result=await chatApplyAutoTrimForPendingBatch(cfg,[],null,{trigger:roundLimitDue?'round_limit':'idle_1h',idleCheck:true});
...
function chatPendingMessageId(){
  chatPendingMessageSeq=(chatPendingMessageSeq+1)%1000000;
  return 'pending-'+Date.now().toString(36)+'-'+chatPendingMessageSeq.toString(36)+'-'+Math.random().toString(36).slice(2,8);
}
function chatConversationTurnId(){
  return 'turn-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,9);
}
function chatEnsurePendingMessageId(message){
  if(!message||typeof message!=='object')return '';
  if(!message.pendingId)message.pendingId=chatPendingMessageId();
  return String(message.pendingId);
}
function chatPendingMessageSignature(message){
  var imageIds=chatMessageImages(message).map(function(img){return String(img.id||chatImageFingerprint(img.dataUrl)||'')});
  return [Number(message&&message.ts)||0,String((message&&message.text)||''),imageIds.join(',')].join('|');
}
function chatPreparePendingBatch(messages){
  return (messages||[]).map(function(message){
    return {id:chatEnsurePendingMessageId(message),signature:chatPendingMessageSignature(message)};
  });
}
function chatResolvePendingBatch(batch){
  var pool=chatPendingMessages(),used=new Set(),resolved=[];
  (batch||[]).forEach(function(item){
    var found=pool.find(function(message){
      if(used.has(message))return false;
      if(item.id&&String(message.pendingId||'')===item.id)return true;
      return chatPendingMessageSignature(message)===item.signature;
    });
    if(found){used.add(found);resolved.push(found)}
  });
  return resolved;
}
function chatInsertMessagesBeforePending(messages){
  messages=Array.isArray(messages)?messages.filter(Boolean):[];
  if(!messages.length)return chatMessages.length;
  var insertIndex=chatMessages.findIndex(function(message){return message&&message.role==='pending_user'});
  if(insertIndex<0)insertIndex=chatMessages.length;
  if(chatEditingIndex>=insertIndex)chatEditingIndex+=messages.length;
  chatMessages.splice.apply(chatMessages,[insertIndex,0].concat(messages));
  return insertIndex;
}
...
function chatStageUserMessage(text,images){
  ...
  var msg={role:'pending_user',text:text,images:images,ts:Date.now(),pendingId:chatPendingMessageId()};
...
function chatPendingGestureReset(){
  var gesture=chatPendingGesture;
  if(gesture.pressTimer){clearTimeout(gesture.pressTimer);gesture.pressTimer=0;}
  if(gesture.row)gesture.row.classList.remove('pending-swipe-active','pending-long-press');
  chatPendingGesture={row:null,message:null,startX:0,startY:0,dx:0,pointerId:null,pressTimer:0,moved:false,longPressed:false};
}
function chatPendingGestureStart(e){
  ...
  chatPendingGesture.message=message;
  chatPendingGesture.startX=Number(point.clientX)||0;
  chatPendingGesture.startY=Number(point.clientY)||0;
  chatPendingGesture.pointerId=e.pointerId===undefined?null:e.pointerId;
  if(chatPendingGesture.pointerId!==null&&row.setPointerCapture){
    try{row.setPointerCapture(chatPendingGesture.pointerId)}catch(err){}
  }
  chatPendingGesture.pressTimer=setTimeout(function(){
    if(!chatPendingGesture.row||chatPendingGesture.moved)return;
    chatPendingGesture.longPressed=true;
    chatPendingGesture.row.classList.add('pending-long-press');
    chatPendingSuppressClickUntil=Date.now()+500;
    chatDeletePendingMessage(chatMessages.indexOf(message));
  },520);
}
function chatPendingGestureMove(e){
  var gesture=chatPendingGesture;
  if(!gesture.row)return;
  if(gesture.pointerId!==null&&e.pointerId!==undefined&&e.pointerId!==gesture.pointerId)return;
  var point=e.touches&&e.touches[0]||e;
  var dx=(Number(point.clientX)||0)-gesture.startX;
  var dy=(Number(point.clientY)||0)-gesture.startY;
  if(Math.abs(dx)>8||Math.abs(dy)>8){
    gesture.moved=true;
    if(gesture.pressTimer){clearTimeout(gesture.pressTimer);gesture.pressTimer=0;}
  }
  if(Math.abs(dx)>Math.abs(dy)&&dx<0){
    gesture.dx=Math.max(-88,dx);
    gesture.row.style.setProperty('--pending-swipe-x',gesture.dx+'px');
    gesture.row.classList.add('pending-swipe-active');
    if(e.cancelable)e.preventDefault();
  }
}
function chatPendingGestureEnd(e){
  var gesture=chatPendingGesture;
  if(!gesture.row)return;
  if(gesture.pointerId!==null&&e&&e.pointerId!==undefined&&e.pointerId!==gesture.pointerId)return;
  var row=gesture.row,message=gesture.message,dx=gesture.dx,longPressed=gesture.longPressed,moved=gesture.moved;
  var index=chatMessages.indexOf(message);
  chatPendingGestureReset();
  if(index<0)return;
  if(dx<=-62){
...
function chatAttachPendingGestures(){
  var box=chatMessagesBox();
  if(!box||box.__ckPendingGesturesAttached)return;
  box.__ckPendingGesturesAttached=true;
...
function chatStartEditMessage(i){
  ...
  if(m.role!=='user'&&m.role!=='pending_user')return;
  if(chatSending&&m.role!=='pending_user')return;
  var input=document.getElementById('chat-input');
  var btn=document.getElementById('chat-send-btn');
  if(!input)return;
...
function chatSaveEditedMessage(){
  ...
  if(m.role==='user'){
    ...
    chatInvalidateTransportForEdit();
  }
  input.value='';
  chatAutosizeInput(input);
  chatExitEditMode();
...
function chatMessageThinkingText(m){
  if(!m)return '';
  var pseudo=String(chatSplitThinkingText(String(m.text||'')).thinking||'').trim();
  return [String(m.thinking||'').trim(),pseudo].filter(Boolean).join('\n\n');
}
...
function chatSplitOutsideCodeBlocks(text){
  ...
  function flush(forceCode){
    var value=buf.join('\n').trim();
    if(value)blocks.push({text:value,code:forceCode===true||inFence});
    buf=[];
  }
  lines.forEach(function(line){
    var mark=line.match(/^\s*(`{3,}|~{3,})/);
    if(mark){
      if(!inFence){
        flush();
        inFence=true;
        fence=mark[1].charAt(0);
      }
      buf.push(line);
      if(inFence&&mark[1].charAt(0)===fence&&buf.length>1){
        inFence=false;
        fence='';
        flush(true);
      }
...
function chatHasUnclosedCodeBlock(text){
  var inFence=false,fence='';
  String(text||'').replace(/\r\n/g,'\n').split('\n').forEach(function(line){
    var mark=line.match(/^\s*(`{3,}|~{3,})/);
    if(!mark)return;
    var kind=mark[1].charAt(0);
    if(!inFence){inFence=true;fence=kind;}
    else if(kind===fence){inFence=false;fence='';}
  });
  return inFence;
}
...
function chatRecordFirstRenderedLatency(latency){
  ...
  latency.panel_first_rendered_char_ms=renderedAt;
  latency.first_delta_to_render_ms=latency.panel_first_delta_ms?Math.max(0,renderedAt-latency.panel_first_delta_ms):null;
  latency.stream_done_to_render_ms=latency.panel_stream_done_ms?Math.max(0,renderedAt-latency.panel_stream_done_ms):null;
  chatDebug('latency',{
    phase:'render',
    debug_id:latency.debug_id||'',
    panel_first_rendered_char_ms:renderedAt,
    first_delta_to_render_ms:latency.first_delta_to_render_ms,
    stream_done_to_render_ms:latency.stream_done_to_render_ms
  });
  return renderedAt;
}
function chatAssistantRevealDelay(){
  return CHAT_ASSISTANT_REVEAL_MIN_DELAY+Math.floor(Math.random()*(CHAT_ASSISTANT_REVEAL_MAX_DELAY-CHAT_ASSISTANT_REVEAL_MIN_DELAY+1));
}
function chatIsAssistantRevealPending(message){
  return !!(chatAssistantRevealQueue&&chatAssistantRevealQueue.hidden&&chatAssistantRevealQueue.hidden.has(message));
}
function chatMarkStaggeredMessageFresh(message){
  var key=chatMessageAnimKey(message);
  if(!key)return;
  chatFreshMessageKeys.add(key);
  chatStaggeredMessageKeys.add(key);
  setTimeout(function(){
    chatFreshMessageKeys.delete(key);
    chatStaggeredMessageKeys.delete(key);
  },1200);
}
function chatScheduleAssistantReveal(){
  var queue=chatAssistantRevealQueue;
  if(!queue||queue.timer||!queue.hidden.size)return;
  queue.timer=setTimeout(function(){
    if(chatAssistantRevealQueue!==queue)return;
    queue.timer=0;
    var message=queue.messages[queue.nextIndex++];
    if(message){
      queue.hidden.delete(message);
  ...
  },chatAssistantRevealDelay());
}
function chatFlushAssistantRevealQueue(options){
  var queue=chatAssistantRevealQueue;
...
function chatRequestWasAborted(err,signal){
  return !!(signal&&signal.aborted);
}
function chatAbortableSleep(ms,signal){
  if(signal&&signal.aborted)return Promise.reject(chatCreateAbortError());
  return new Promise(function(resolve,reject){
    var timer=0;
    function cleanup(){
      if(signal&&signal.removeEventListener)signal.removeEventListener('abort',onAbort);
...
function chatContainsPlatformExitError(value){
  return String(value||'').indexOf(CHAT_PLATFORM_EXIT_ERROR)>=0;
}
function chatCreateRequestFailure(message,retryable){
  var err=new Error(String(message||'请求失败'));
  if(retryable)err.chatRetryableBeforeContent=true;
  return err;
}
function chatMarkNetworkFailure(err){
  if(!err||typeof err!=='object')err=new Error(String(err||'Network request failed'));
  try{err.chatNetworkFailure=true}catch(e){}
  return err;
}
async function chatFetchWithSilentRetry(url,requestInit,consumeResponse){
  var signal=requestInit&&requestInit.signal;
  for(var attempt=0;;attempt++){
    if(signal&&signal.aborted)throw chatCreateAbortError();
    var attemptState={receivedValidContent:false};
    try{
      var resp;
      try{
      ...
      if(!resp.ok){
        var errText='';
        try{errText=await resp.text()}catch(readError){throw chatMarkNetworkFailure(readError)}
        var retryableStatus=resp.status===502||resp.status===503||resp.status===504;
        throw chatCreateRequestFailure(
          'HTTP '+resp.status+(errText?(' '+errText.slice(0,500)):''),
          retryableStatus||chatContainsPlatformExitError(errText)
        );
      }
      await consumeResponse(resp,attemptState);
      return;
    }catch(err){
...
function chatAppendAssistantReplies(rawText,recallInfo,toolEvents,opts){
  opts=opts||{};
  var parts=chatSplitAssistantReplies(rawText,!(opts.splitAssistantReplies===false));
  var tools=chatCloneToolEvents(toolEvents);
  var nativeThinking=String(opts.thinking||'').trim();
  if(!parts.length&&(tools.length||nativeThinking))parts=[''];
  if(!parts.length)return Promise.resolve();
  chatFlushAssistantRevealQueue({render:false});
  ...
  // 挂在第一条是因为整组是一起换掉的，第一条就是这一轮回复的锚点。
  var carried=Array.isArray(opts.replyVariants)?opts.replyVariants.filter(Boolean):[];
  if(carried.length){
    var mine=chatReplyVariantFromMessages(messages);
    var all=chatTrimReplyVariants(carried.concat(mine?[mine]:[]));
    if(all.length>1){
      messages[0].replyVariants=all;
      messages[0].replyVariantIndex=all.length-1;
    }
  }
...
function chatToggleDebugSettings(){
  ...
  var debug=document.getElementById('chat-side-debug');
  var isDebugOpen=!!(el&&el.classList.contains('open')&&debug&&debug.classList.contains('active'));
  if(isDebugOpen){
    chatToggleSettings(false);
    return;
  }
  chatSwitchSideTab('debug');
  chatToggleSettings(true);
}
...
function chatToggleSettings(force,silent){
  ...
  var trigger=document.getElementById('chat-settings-open');
  if(trigger)trigger.setAttribute('aria-expanded',open?'true':'false');
  if(!silent){
    var cfg=chatMergeLiveToggleState(chatLoadConfig());
...
document.addEventListener('click',function(e){
  ...
  var pendingBubble=e.target.closest('.chat-bubble.pending');
  if(pendingBubble){
    if(Date.now()<chatPendingSuppressClickUntil)return;
...
document.addEventListener('keydown',function(e){
  ...
  if(document.querySelector('.chat-settings.open')){
    e.preventDefault();
    chatToggleSettings(false);
    return;
  }
...
function chatScrollJumpControls(){return document.getElementById('chat-scroll-jumps')}
function chatUpdateScrollJumpState(){var c=chatScrollJumpControls(),b=chatMessagesBox();if(!c||!b)return;var t=c.querySelector('[data-chat-scroll-edge="top"]'),d=c.querySelector('[data-chat-scroll-edge="bottom"]');if(t)t.disabled=b.scrollTop<=2;if(d)d.disabled=b.scrollHeight-b.scrollTop-b.clientHeight<=2}
function chatHideScrollJumps(){var c=chatScrollJumpControls();if(!c)return;if(c.contains(document.activeElement)&&document.activeElement.blur)document.activeElement.blur();c.classList.remove('show');c.setAttribute('aria-hidden','true')}
function chatScheduleScrollJumpHide(){if(chatScrollJumpTimer)clearTimeout(chatScrollJumpTimer);chatScrollJumpTimer=setTimeout(chatHideScrollJumps,CHAT_SCROLL_JUMP_VISIBLE_MS)}
function chatRevealScrollJumps(){if(!document.body.classList.contains('chat-active'))return;var c=chatScrollJumpControls();if(!c)return;c.classList.add('show');c.setAttribute('aria-hidden','false');chatUpdateScrollJumpState();chatScheduleScrollJumpHide()}
...
function chatJumpToEdge(edge,event){if(event)event.preventDefault();var b=chatMessagesBox();if(!b)return;var top=edge==='top'?0:b.scrollHeight;try{b.scrollTo({top:top,behavior:ckPrefersReducedMotion()?'auto':'smooth'})}catch(e){b.scrollTop=top}if(edge!=='top')chatSetNewMessageHint(false);chatRevealScrollJumps();setTimeout(chatUpdateScrollJumpState,400)}
...
function chatRenderMessages(opts){
  ...
  if(!chatMessages.length){
    box.innerHTML='<div class="chat-welcome"><b>新的对话</b><p>发条消息，聊聊此刻。</p></div>';
    chatRenderPendingBar();
  ...
  if(!existing.length){
    box.innerHTML=chatMessages.map(function(m,i){return chatIsAssistantRevealPending(m)?'':chatRenderMessageRow(m,i)}).join('');
  }else{
    var rowMap={};
    existing.forEach(function(row){
      var idx=row.getAttribute('data-chat-index');
      if(idx===null||idx===''){
        if(opts.removeEphemeral===true||!row.classList.contains('chat-ephemeral-row'))row.remove();
        return;
      }
      rowMap[idx]=row;
    });
    for(var oldIndex in rowMap){if(Number(oldIndex)>=chatMessages.length)rowMap[oldIndex].remove()}
    for(var i=0;i<chatMessages.length;i++){
      var m=chatMessages[i];
      var row=rowMap[String(i)];
      if(chatIsAssistantRevealPending(m)){
        if(row)row.remove();
        continue;
      }
      var key=chatMessageRenderKey(m,i);
      if(!row){
        chatInsertRenderedMessageRow(box,chatRenderMessageRow(m,i),i);
        continue;
      }
...
function chatCacheExpiryInfo(){
  // 用当前供应商维护的策略；轮询状态未返回前不猜，避免提示错误 TTL。
  var strategy=chatCacheNoticeStrategy(chatLoadConfig()||{});
  if(!strategy)return {ttlMs:0,text:''};
  var meta=chatCacheStrategyMeta(strategy);
...
function chatEnsureCacheExpiryNotice(){
  ...
  var cfg=chatLoadConfig()||{};
  if(
    chatCacheNoticeStrategy(cfg)==='prefix_24h'
    && chatAutoTrimConfigFrom(cfg).prefixSilent
  )return false;
...
/* ---- 助手回复的版本翻页 ---- */
// 重新生成不再把上一条答案删掉。旧答案整组存成一个版本，新答案追加成下一个版本，
// 气泡底下出现 ◂ ▸ 翻页——和用户消息双击改文案那套是一个交互。
// 存的是"整组消息"而不是一段文本：分条开着时一轮回复会拆成好几个气泡，
// 只存文本再拆一次就会和当时看到的不一样。翻页＝整组换掉。
var CHAT_REPLY_VARIANT_MAX=6;
function chatCloneReplyMessages(list){
  try{
...
function chatAssistantGroupRange(i){
  if(!chatMessages[i]||chatMessages[i].role!=='assistant')return null;
  var start=i,end=i;
  while(start>0&&chatMessages[start-1]&&chatMessages[start-1].role==='assistant')start--;
  while(chatMessages[end+1]&&chatMessages[end+1].role==='assistant')end++;
  return {start:start,end:end};
}
function chatNormalizeReplyVariant(v){
  v=v&&typeof v==='object'?v:{};
  var messages=chatCloneReplyMessages(v.messages).filter(function(m){return m&&m.role==='assistant'});
  if(!messages.length)return null;
  messages.forEach(function(m){delete m.replyVariants;delete m.replyVariantIndex});
  return {ts:Number(v.ts)||Number(messages[0].ts)||0,messages:messages};
}
function chatStoredReplyVariants(m){
  return (Array.isArray(m&&m.replyVariants)?m.replyVariants:[]).map(chatNormalizeReplyVariant).filter(Boolean);
}
function chatReplyVariantFromMessages(list){
  return chatNormalizeReplyVariant({messages:list});
}
function chatReplyVariantInfo(i){
  var range=chatAssistantGroupRange(i);
  if(!range)return null;
  var head=chatMessages[range.start];
  var variants=chatStoredReplyVariants(head);
  if(variants.length<2)return null;
  var index=Math.max(0,Math.min(variants.length-1,Math.floor(Number(head.replyVariantIndex)||0)));
  return {range:range,head:head,variants:variants,index:index,total:variants.length};
}
// 点重新生成时先把当前这一组存下来：已经有版本记录就沿用，没有就现造第一版。
function chatCollectReplyVariants(range){
  if(!range)return [];
  var stored=chatStoredReplyVariants(chatMessages[range.start]);
  if(stored.length)return stored;
  var current=chatReplyVariantFromMessages(chatMessages.slice(range.start,range.end+1));
  return current?[current]:[];
}
function chatTrimReplyVariants(list){
  list=(list||[]).filter(Boolean);
  return list.length>CHAT_REPLY_VARIANT_MAX?list.slice(list.length-CHAT_REPLY_VARIANT_MAX):list;
}
function chatAssistantVariantSignature(m,i){
  if(!m||m.role!=='assistant')return '';
  var range=chatAssistantGroupRange(i);
  if(!range)return '';
  var head=chatMessages[range.start];
  if(!head||!Array.isArray(head.replyVariants))return '';
  return head.replyVariants.length+':'+(Math.floor(Number(head.replyVariantIndex)||0));
}
function chatReplyVariantNavHtml(i){
  var info=chatReplyVariantInfo(i);
  if(!info)return '';
  ...
  return '<div class="chat-version-nav assistant" aria-label="回复版本">'+
    '<button class="chat-version-btn chat-reply-variant-btn" type="button" data-i="'+i+'" data-dir="-1" title="上一版回复"'+prevDisabled+'>◂</button>'+
    '<span>'+esc(String(info.index+1))+'/'+esc(String(info.total))+'</span>'+
    '<button class="chat-version-btn chat-reply-variant-btn" type="button" data-i="'+i+'" data-dir="1" title="下一版回复"'+nextDisabled+'>▸</button>'+
    '</div>';
}
function chatSwitchReplyVariant(i,dir){
  if(chatSending)return;
  if(chatEditingIndex>=0)chatCancelEdit();
  var info=chatReplyVariantInfo(i);
  if(!info)return;
  var target=Math.max(0,Math.min(info.total-1,info.index+(Number(dir)||0)));
  if(target===info.index)return;
  var replacement=chatCloneReplyMessages(info.variants[target].messages);
  if(!replacement.length)return;
  // 时间戳和 turnId 一律沿用"当前这一组"的：换的是这一轮的答案，不是插了一段旧对话。
  // 不改的话轮次分组（chat-history 按 turnId 数轮）和时间顺序都会错。
  var oldGroup=chatMessages.slice(info.range.start,info.range.end+1);
  var turnId=String((oldGroup[0]&&oldGroup[0].turnId)||'');
  var baseTs=Number(oldGroup[0]&&oldGroup[0].ts)||Date.now();
  replacement.forEach(function(msg,index){
    var old=oldGroup[index];
    msg.ts=Number(old&&old.ts)||(baseTs+index);
    if(turnId)msg.turnId=turnId;
  });
  replacement[0].replyVariants=info.variants;
  replacement[0].replyVariantIndex=target;
  Array.prototype.splice.apply(chatMessages,[info.range.start,oldGroup.length].concat(replacement));
  // 翻页把屏幕上的内容换了，隐藏的 transport 就和显示的不一致了，必须让它作废：
  // 下一轮改走 window_messages，也就是"以页面上看到的为准"重建历史，绝不会出现
  // 「显示 A、发给模型 B」。前面那些用户消息网关会按原文哈希还原成冻结版本，
  // 前缀仍然逐字一致，真正要重算的只有被换掉的这一段往后——这已经是物理下限。
  chatInvalidateTransportForEdit();
  chatSaveLocalMessages();
  chatRenderMessages({respectUserScroll:true});
}
...
function chatRenderMessageRow(m,i){
  ...
  var assistantStopped=role==='assistant'&&!!(m&&(m.stopped===true||/^\s*(?:（已停止）|\(已停止\))/i.test(String(m.text||''))));
  var assistantError=role==='assistant'&&!assistantStopped&&!!(m&&(
    m.failed===true||m.error===true||m.is_error===true||m.status==='error'||
  ...
  var usage=(role==='assistant'&&isGroupLast)?chatAssistantUsageHtml(m):'';
  ...
  // 助手这边翻的是"整轮回复的版本"（重新生成攒出来的），挂在这一组的最后一条上，
  // 和重新生成按钮同一行位置，避免用户在中间某个气泡下面找翻页。
  if(!versionNav&&role==='assistant'&&isGroupLast)versionNav=chatReplyVariantNavHtml(i);
...
function chatIsMessageGroupLast(i,role){
  var next=chatMessages[i+1];
  if(!next||next.role==='notice')return true;
  var nextRole=next.role==='pending_user'?'user':(next.role==='user'?'user':(next.role==='system'?'system':'assistant'));
  return nextRole!==role;
}
function chatMessageTimingHtml(m,role,showTimestamp){
  var bits=[],ts=Number(m&&m.ts)||0;
  ...
  var separator=role==='assistant'?'<span class="chat-msg-separator" aria-hidden="true">·</span>':'';
  return '<div class="chat-msg-time'+(role==='user'?' chat-msg-meta':'')+'">'+bits.join(separator)+'</div>';
}
function chatUserMessageMetaHtml(m,showTimestamp){
...
function chatAssistantUsageHtml(m){
  if(!chatUsageStatsEnabled())return '';
  if(!m||m.role!=='assistant')return '';
  var input=Math.max(0,Number(m.tkIn)||0);
  var output=Math.max(0,Number(m.tkOut)||0);
  var read=Math.max(0,Number(m.tkRead)||0);
  var create=Math.max(0,Number(m.tkCreate)||0);
...
function chatApplyCacheTick(userIndex,usage,userBubble){
  ...
  if(userBubble&&chatShouldShowMessageStatus()){
    ...
    if(!meta){
      ...
      meta.className='chat-msg-time chat-msg-meta';
...
function chatPlaceAssistantBubbleBeforePending(bubble){
  var row=bubble&&bubble.parentNode;
  var box=chatMessagesBox();
  if(!row||!box)return;
  var firstPending=box.querySelector('.chat-msg-row.pending');
  if(firstPending)box.insertBefore(row,firstPending);
...
function chatAddBubble(role,text,persist){
  ...
  if(!persist)row.classList.add('chat-ephemeral-row');
  ...
  if(role==='user'){
    ...
    if(role==='assistant'&&!text){
      el.classList.add('streaming-empty');
      el.innerHTML='';
    }
  }
  row.appendChild(el);
  if(role==='user'){
    var metaWrap=document.createElement('template');
    metaWrap.innerHTML=chatUserMessageMetaHtml({role:'user',text:text||'',images:[],cacheHit:false,ts:ts});
    if(metaWrap.content&&metaWrap.content.firstChild)row.appendChild(metaWrap.content.firstChild);
  }else{
...
async function chatRenameCurrent(){
  var s=chatCurrentSession();
  var name=await ckPromptDialog('重命名对话',s.title||'',{placeholder:'对话名称',confirmText:'保存'});
  if(name===null)return;
  ...
  if(input){
    ...
    input.addEventListener('keydown',function(e){
      if(e.key==='Enter'&&!e.shiftKey){
        e.preventDefault();
        if(chatEditingIndex>=0)chatSaveEditedMessage();
        else chatStoreDraftMessage({keepFocus:true});
      }
    });
  }
}
async function chatSendMessage(){
  ...
  if(text||chatDraftImages.length||chatDraftFiles.length||chatPendingMessages().length||chatFailedUserMessages().length){
    ...
    if(text||imageSnapshot.length){
      ...
      chatMarkMessageFresh(optimistic);
      var optimisticIndex=chatMessages.length-1;
      var messageBox=chatMessagesBox();
      if(messageBox&&(messageBox.querySelector('.chat-welcome')||messageBox.querySelector('.empty-state')))messageBox.innerHTML='';
      if(optimisticIndex>0)chatUpdateMessageRowOnly(optimisticIndex-1);
...
function chatRegenerateFromUser(i){
  ...
  if(m&&m.role==='assistant'){
    for(i=i-1;i>=0;i--){
      if(chatMessages[i]&&chatMessages[i].role==='user'){m=chatMessages[i];break;}
    }
  }
  if(!m||m.role!=='user'||!chatMessageHasContent(m))return;
  // 旧答案不删了：整组存成"上一版"带着走，新答案回来以后追加成下一版，
  // 气泡底下就出现 ◂ ▸ 翻页。这里仍然要把它从 chatMessages 里摘掉——
  // 这一轮要重发，不能把上一版答案当成历史再发给模型。
  var carry=[];
  if(chatMessages[i+1]&&chatMessages[i+1].role==='assistant'){
    carry=chatCollectReplyVariants(chatAssistantGroupRange(i+1));
  }
  chatMessages=chatMessages.slice(0,i+1);
  chatMessages[i].role='pending_user';
  chatMessages[i].regenerateRequest=true;
  chatMessages[i].regenerateCutoff=i;
  if(carry.length)chatMessages[i].replyVariantsCarry=chatTrimReplyVariants(carry).slice(-Math.max(1,CHAT_REPLY_VARIANT_MAX-1));
  // 收不到新的就保留原来存着的那份：上一次重新生成被停/失败时答案已经摘掉了，
  // 这里再删一遍就等于把旧答案彻底弄丢。
...
async function chatSubmitPendingMessages(options){
  ...
  var requestTurnId=String((pending.find(function(message){return message&&message.turnId})||{}).turnId||'')||chatConversationTurnId();
  pending.forEach(function(message){if(message)message.turnId=requestTurnId});
  if(requestState)requestState.turnId=requestTurnId;
  ...
  var carriedReplyVariants=[];
  var carriedReplyOwners=[];
  pending.forEach(function(m){
    var i=chatMessages.indexOf(m);
  ...
  if(regenerateRequest)windowMessagesForRequest=chatWindowContextMessages();
  ...
  if(!speechEnabledForRequest&&chatDiscardPendingSpeechPreference(currentSession))chatSaveSessions();
  var pendingSpeechActivationId='';
  if(speechEnabledForRequest&&(!currentSession.speechPreferenceRetryAtBoundary||speechBoundaryNow)){
    pendingSpeechActivationId=String(currentSession.speechPreferencePendingActivationId||'')||
      String(trimResult.speechPreferenceActivationId||'');
  }
  ...
  if(speechBoundaryNow){
    body.cache_rebuild_boundary=String(
      trimResult.trigger||currentSession.speechPreferencePendingBoundaryReason||'manual'
    );
  }
  ...
  // 以前这里还有一个「同窗口全量上下文」开关在把关。它已经删掉：网关每轮都回
  // transport_messages，所以从第二轮起这个分支根本走不到；真走到的时候（首轮、
  // 重新生成、截断后、切版本）另外几个条件已经各自成立，把它关掉只会让上下文变少、
  ...
  try{
    ...
    await chatFetchWithSilentRetry(chatEndpoint(cfg),{
      ...
      function handleStreamEvent(ev,data){
        ...
        if(ev==='error'){
          var streamErrorText=data&&typeof data==='object'
            ...
            : String(data||'');
          if(!streamErrorText)streamErrorText=JSON.stringify(data||{});
        ...
        if(ev==='delta'){
          ...
          if(ev==='done'){
            ...
            if(completedSession.cacheRebuildPending===true){
              ...
              if(!completedSession.speechPreferencePendingActivationId){
                completedSession.speechPreferencePendingBoundaryReason='';
              }
      ...
      while(true){
        ...
        buffer+=chunk;
        buffer=chatParseSse(buffer,handleStreamEvent);
      ...
      if(tail){
        if(!attemptState.receivedValidContent)beforeContentRaw=(beforeContentRaw+tail).slice(-4000);
        buffer+=tail;
      }
      if(buffer.trim())chatParseSse(buffer+'\n\n',handleStreamEvent);
      if(streamError)throw streamError;
      if(!attemptState.receivedValidContent&&chatContainsPlatformExitError(beforeContentRaw)){
        throw chatCreateRequestFailure(CHAT_PLATFORM_EXIT_ERROR,true);
      }
...
function switchPanelTab(tab,opts) {
  ...
  if(tab==='facts'){
    if(!factLibraryData)loadFactLibrary(true,true);
...
function apiConfigErrorHtml(reason){
  ...
  return '<div class="entity-error">读不到配置。'+esc(reason||'请确认 CK 网关面板 Key 正确。')+'</div>'+
    ...
    '<button class="btn btn-blue btn-sm" type="button" onclick="requestApiKeyDialog(&quot;CK 网关面板 Key&quot;).then(function(ok){if(ok){apiProvidersLoaded=false;renderApiConfig()}})">重新输入 Key</button>'+
    '<button class="btn btn-outline btn-sm" type="button" onclick="apiProvidersLoaded=false;renderApiConfig()">重新读取</button>'+
    '</div>';
}
...
function normalizedProviderBase(value){
...
function normalizeProvider(p){
  ...
  return {
    ...
    api_type:providerNormalizeApiType(p.api_type||p.apiType,p.url),
...
function providerNormalizeApiType(value,url){
  var raw=String(value||'').trim().toLowerCase().replace(/-/g,'_');
  if(raw==='claude'||raw==='anthropic'||raw==='messages'||raw==='anthropic_messages')return 'claude';
  if(raw==='openai'||raw==='chat'||raw==='chat_completions'||raw==='openai_chat')return 'openai';
  var base=String(url||'').trim().toLowerCase();
  return /nowcoding\.ai|api\.anthropic\.com|anthropic\.com|\/messages(?:\/|$)/.test(base)?'claude':'openai';
}
...
function apiPollingItemFor(p,model){
  ...
  return {
    ...
    api_type:providerNormalizeApiType(p&&p.api_type,p&&p.url),
...
function apiPollingRevision(polling,items){
  ...
  var payload=JSON.stringify({
    ...
    order:items.filter(function(x){return x.available}).map(function(x){
       return [x.provider_id,x.model,x.cache_strategy,x.api_type,providerFingerprint(x.provider)];
    })
  });
  var hash=2166136261;
...
function chatPollingView(){
  ...
  var view={enabled:false,revision:'',showMessageStatus:false,showBillingPrice:false,pricing:null};
  if(typeof apiProvidersLoaded!=='undefined'&&apiProvidersLoaded){
    ...
    try{
      ...
      view={
        ...
        pricing:(raw.pricing&&typeof raw.pricing==='object'&&!Array.isArray(raw.pricing))?raw.pricing:null
      };
    }catch(e){}
  }
...
// 轮询关闭时照常显示 √ 和价格；开启轮询后这两样各由轮询页自己的勾选框决定（默认关）。
// 价格头上还压着一道总闸：设置页关掉计费，轮询这边勾了也不显示。
function chatShouldShowMessageStatus(){
  var view=chatPollingView();
  if(view.enabled!==true)return true;
  return view.showMessageStatus===true;
}
function chatShouldShowBillingPrice(){
  if(!chatBillingEnabled())return false;
  var view=chatPollingView();
  if(view.enabled!==true)return true;
  return view.showBillingPrice===true;
}
// 渲染路径已经按上面两个判断决定要不要输出节点，body 上这两个类只是给 CSS
// 留一道兜底（增量刷新、旧节点残留时不至于漏出来）。
function chatApplyDisplayGateClasses(){
...
function normalizeApiProviders(raw){
  ...
  var rewriteSlot=apiGroupSlot('recall_rewrite');
  if(!findLibraryProvider(rewriteSlot.current)){
    var rewriteBase=normalizedProviderBase(apiDirectValue('RECALL_REWRITE_BASE'));
    var rewriteModel=apiDirectValue('RECALL_REWRITE_MODEL').trim();
    var matches=rewriteBase?providerLibraryList().filter(function(p){return normalizedProviderBase(p.url)===rewriteBase}):[];
    if(matches.length>1&&rewriteModel){
      var modelMatches=matches.filter(function(p){return cleanModelList(p.models,p.model).indexOf(rewriteModel)>=0});
      if(modelMatches.length===1)matches=modelMatches;
    }
...
function providerCategories(){
  var seen={};
  return providerLibraryList().map(function(provider){return String(provider.category||'').trim()}).filter(function(category){if(!category||seen[category])return false;seen[category]=true;return true}).sort(function(a,b){return a.localeCompare(b,'zh-CN')});
}
var API_PROVIDER_CATEGORY_VIEW_KEY='ckApiProviderCategoryViewV1';
function providerCategoryView(){try{return JSON.parse(localStorage.getItem(API_PROVIDER_CATEGORY_VIEW_KEY)||'{}')||{}}catch(e){return {}}}
function toggleProviderCategory(button){
...
function renderApiPolling(){
  ...
  html+='<div class="api-polling-options">'+
    ...
    '<div class="api-polling-display'+(draft.enabled?'':' disabled')+'">'+
      ...
      '<p>只在开启轮询后起作用：轮询开着时默认两样都不显示，勾上才显示。关闭轮询一律照常显示。价格还要看聊天页「设置」里的计费总闸——那边关掉，这里勾了也不显示。</p>'+
    '</div>'+
    '</div>';
  html+='<p class="api-polling-note">每个候选 API 单次最多等待 60 秒，超时立即切下一个；开启轮询后回复会等整段生成完再显示。</p>';
...
function providerCardHtml(p){
  ...
  var apiType=providerNormalizeApiType(p.api_type,p.url);
...
function readProvCard(card){
  ...
  return {id:id,name:v('.prov-name-input'),category:v('.prov-category-input'),note:v('.prov-note-input'),url:v('.prov-url'),key:v('.prov-key'),model:v('.prov-model'),models:old&&Array.isArray(old.models)?old.models:[],
    ...
    api_type:providerNormalizeApiType(v('.prov-api-type'),v('.prov-url'))};
}
...
function addProvider(){
  switchApiTab('providers');
  var id=newProvId();
  apiProviderLibrarySlot().providers.push({id:id,name:'',category:'',note:'',url:'',key:'',model:'',models:[],cache_strategy:'',pricing:null,api_type:'openai'});
  renderApiConfig();
  setTimeout(function(){
...
function fetchModelsForProvider(p){
  return panelDataFetch(PROVIDER_MODELS_URL+'?_t='+Date.now(),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:p.url,key:p.key,api_type:providerNormalizeApiType(p.api_type,p.url)})}).then(function(r){return r.json().then(function(j){return {ok:r.ok,j:j}},function(){return {ok:r.ok,j:{}}})}).then(function(res){
...
function fetchProviderModels(btn){
  ...
  p.cache_strategy=d.cache_strategy;p.pricing=d.pricing;p.api_type=providerNormalizeApiType(d.api_type,d.url);
...
function saveProvider(btn){
  ...
  var p=findLibraryProvider(d.id);
  if(!p){
    var fp=providerFingerprint(normalizeProvider(d));
    var list=apiProviderLibrarySlot().providers;
  ...
  p.pricing=d.pricing;
  p.api_type=providerNormalizeApiType(d.api_type,d.url);
...
function loadApiProviders(opts){
  ...
  apiProvidersLoadInFlight=keyCfgFetch(undefined,opts||{}).then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.json()}).then(function(d){
    ...
    apiProvidersLoadInFlight=null;
    return result;
  },function(err){
    apiProvidersLoadInFlight=null;
    throw err;
  });
  return apiProvidersLoadInFlight;
}
