...
var CK_PANEL_VERSION=window.CK_PANEL_VERSION||'chat-v109-race-render-hardening';
...
function chatAddBubble(role,text,persist){
  ...
  if(role==='user'){
    ...
    if(role==='assistant'&&!text){
      ...
      el.innerHTML='<span class="chat-typing" aria-label="对方正在输入"><i></i><i></i><i></i></span>';
    }
  }
  row.appendChild(el);
  if(role==='user'){
...
async function chatSubmitPendingMessages(options){
  ...
  pending=chatPendingMessages();
  if(!document.querySelector('.chat-typing'))out=chatAddBubble('assistant','',false);
